...
IncrementalEstimator::IncrementalEstimator(
    size_t groupId, const Options& options,
    const LinearSolverOptions& linearSolverOptions,
    const OptimizerOptions& optimizerOptions)
    : _options(options),
      _margGroupId(groupId),
      _optimizer(std::make_shared<Optimizer>(optimizerOptions)),
      _problem(std::make_shared<IncrementalOptimizationProblem>()),
  ...
  optOptions.linearSystemSolver =
      std::make_shared<LinearSolver>(linearSolverOptions);
  optOptions.trustRegionPolicy = std::make_shared<TrustRegionPolicy>();
...
IncrementalEstimator::IncrementalEstimator(const sm::PropertyTree& config)
  ...
  _optimizer = std::make_shared<Optimizer>(
      ...
      std::make_shared<TrustRegionPolicy>());
  ...
  _options.infoGainDelta =
      config.getDouble("infoGainDelta", _options.infoGainDelta);
  _options.checkValidity =
      config.getBool("checkValidity", _options.checkValidity);
...
const IncrementalOptimizationProblem* IncrementalEstimator::getProblem() const {
  return _problem.get();
}

const IncrementalEstimator::Options& IncrementalEstimator::getOptions() const {
  return _options;
}
...
const Eigen::MatrixXd& IncrementalEstimator::getSigma2ThetaObs(
    bool scaled) const {
  if (scaled)
...
const Eigen::VectorXd& IncrementalEstimator::getSingularValues(
    bool scaled) const {
  if (scaled)
...
IncrementalEstimator::ReturnValue IncrementalEstimator::addBatch(
    const BatchSP& problem, bool force) {
  // query the time
  const double timeStart = Timestamp::now();
  ...
  if (_options.checkValidity &&
      (srv.iterations == _optimizer->options().maxIterations ||
...
void IncrementalEstimator::orderMarginalizedDesignVariables() {
  ...
  auto margGroupIt =
      std::find(groupsOrdering.begin(), groupsOrdering.end(), _margGroupId);
