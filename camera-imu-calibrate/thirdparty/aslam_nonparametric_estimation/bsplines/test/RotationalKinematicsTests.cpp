...
struct SMatrixFunctor2 {
  ...
  SMatrixFunctor2(const Eigen::Vector3d& pbar) : pbar_(pbar) {
    Cbar_ = rotation.parametersToRotationMatrix(pbar);
  }
 ...
 public:
  ...
  void testParametersToRotationMatrix() {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      sm::eigen::assertNear(one, Eigen::Matrix3d::Identity(), 1e-10,
                            SM_SOURCE_FILE_POS);
    }
  }
  ...
  void testSMatrix() {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      Eigen::Matrix3d estCvCrossS =
          nd.estimateJacobian(Eigen::Vector3d::Zero());
      Eigen::Matrix3d CvCrossS =
          crossMx(Cv) * rotation.parametersToSMatrix(pbar);
