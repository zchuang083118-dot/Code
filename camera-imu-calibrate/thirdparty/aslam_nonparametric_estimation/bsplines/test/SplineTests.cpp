    ...
    for (int dim = 1; dim < 4; dim++) {
      ...
      for (int derivative = 0; derivative < order; derivative++) {
        for (double t = bs.t_min(); t < bs.t_max(); t += 0.1) {
          ...
          Eigen::MatrixXd estJ =
              nd.estimateJacobian(bs.localCoefficientVector(t));
          Eigen::MatrixXd J;
          Eigen::VectorXd v;
...
TEST(SplineTestSuite, testCoefficientMap) {
  ...
  for (int i = 0; i < bs.numVvCoefficients(); i++) {
    ...
    // Test pass by value...
    Eigen::Map<Eigen::Matrix<double, 5, 1> > m2 =
...
TEST(SplineTestSuite, testGetBi) {
  ...
  for (int i = 0; i <= numTimeSteps; i++) {
    ...
    Eigen::VectorXi localCoefficientVectorIndices =
        bs.localCoefficientVectorIndices(t);
    int firstIndex = localCoefficientVectorIndices[0];
    SM_ASSERT_EQ(std::runtime_error, localCoefficientVectorIndices.size(),
                 order,
                 "localCoefficientVectorIndices has to have exactly "
                     << order << " entries");

    for (int j = 0; j < order; j++) {
      SM_ASSERT_EQ(std::runtime_error, localCoefficientVectorIndices[j],
                   firstIndex + j,
    ...
    for (int j = 0, n = biVector.size(); j < n; j++) {
      if (j < firstIndex) {
        ...
        SM_ASSERT_EQ(std::runtime_error, biVector[j],
                     localBiVector[j - firstIndex],
                     "localBiVector should be a slice of the biVector");
        SM_ASSERT_EQ(std::runtime_error, cumulativeBiVector[j],
                     localCumulativeBiVector[j - firstIndex],
                     "localCumulativeBiVector must be a slice of the "
                     "cumulativeBiVector");
        SM_ASSERT_NEAR(
            std::runtime_error, cumulativeBiVector[j],
            localBiVector.segment(j - firstIndex, order - (j - firstIndex))
                .sum(),
            1e-13,
            "cumulativeBiVector must be the sum of the localBiVector where it "
            "overlaps, but it is not at "
                << (j - firstIndex) << " (localBiVector=" << localBiVector
                << ")");
      } else {
        SM_ASSERT_EQ(std::runtime_error, biVector[j], 0, "at position " << j);
        SM_ASSERT_EQ(std::runtime_error, cumulativeBiVector[j], 0,
                     "at position " << j);
      }
    }
  }
...
//   std::vector<double> knots;
//   knots.insert(knots.begin(),knots_d,knots_d +
//   (sizeof(knots_d)/sizeof(double)));

//   std::vector<double> control;
//   control.insert(control.begin(),control_d,control_d +
//   (sizeof(control_d)/sizeof(double)));
