...
BSplineDesignVariable<D>::BSplineDesignVariable(
    const bsplines::BSpline &bspline)
    : _bspline(bspline) {
  ...
  for (int i = 0; i < _bspline.numVvCoefficients(); ++i) {
    _designVariables.push_back(
        new aslam::backend::DesignVariableMappedVector<D>(
            _bspline.fixedSizeVvCoefficientVector<D>(i)));
  }
}
...
template <int D>
aslam::backend::DesignVariableMappedVector<D> *
BSplineDesignVariable<D>::designVariable(size_t i) {
...
template <int D>
aslam::backend::VectorExpression<D> BSplineDesignVariable<D>::toExpression(
    double tk, int derivativeOrder) {
  ...
  std::shared_ptr<aslam::splines::BSplineVectorExpressionNode<D> > root(
      new aslam::splines::BSplineVectorExpressionNode<D>(
          &_bspline, derivativeOrder, dvs, tk));
...
BSplineDesignVariable<D>::getDesignVariables(double tk) const {
  Eigen::VectorXi dvidxs = _bspline.localVvCoefficientVectorIndices(tk);
  std::vector<aslam::backend::DesignVariable *> dvs;
  for (int i = 0; i < dvidxs.size(); ++i) {
    dvs.push_back(const_cast<aslam::backend::DesignVariableMappedVector<D> *>(
...
void BSplineDesignVariable<D>::addSegment(double t, const Eigen::VectorXd &p) {
  ...
  _designVariables.push_back(new aslam::backend::DesignVariableMappedVector<D>(
      _bspline.fixedSizeVvCoefficientVector<D>(_bspline.numVvCoefficients() -
                                               1)));
...
template <int D>
void BSplineDesignVariable<D>::addSegment2(double t, const Eigen::VectorXd &p,
                                           double lambda) {
