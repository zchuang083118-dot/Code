...
Optimizer2::Optimizer2(const sm::PropertyTree& config,
  ...
  options.convergenceDeltaJ =
      config.getDouble("convergenceDeltaJ", options.convergenceDeltaJ);
  options.convergenceDeltaX =
      config.getDouble("convergenceDeltaX", options.convergenceDeltaX);
  options.maxIterations = config.getInt("maxIterations", options.maxIterations);
  options.doSchurComplement =
      config.getBool("doSchurComplement", options.doSchurComplement);
  options.verbose = config.getBool("verbose", options.verbose);
  options.linearSolverMaximumFails = config.getInt(
      "linearSolverMaximumFails", options.linearSolverMaximumFails);
  ...
  // i.e., directly call the upper constructor
}

Optimizer2::~Optimizer2() {}

/// \brief Set up to work on the optimization problem.
void Optimizer2::setProblem(std::shared_ptr<OptimizationProblemBase> problem) {
  _problem = problem;
}
...
void Optimizer2::initializeTrustRegionPolicy() {
  if (!_options.trustRegionPolicy) {
    _options.verbose&& std::cout
        << "No trust region policy set in the options. Defaulting to "
           "levenberg_marquardt\n";
  ...
  // \todo remove this check when the sparse qr solver supports an augmented
  // diagonal
  if (_solver->name() == "sparse_qr" &&
      _trustRegionPolicy->name() == "levenberg_marquardt") {
    _options.verbose&& std::cout
        << "The sparse_qr solver is not compatible with levenberg_marquardt. "
           "Changing to the dog_leg trust region policy\n";
    _trustRegionPolicy.reset(new DogLegTrustRegionPolicy());
  }

  _options.verbose&& std::cout << "Using the " << _trustRegionPolicy->name()
                               << " trust region policy\n";
}
...
void Optimizer2::initializeLinearSolver() {
  if (!_options.linearSystemSolver) {
    _options.verbose&& std::cout
        << "No linear system solver set in the options. Defaulting to the "
           "sparse_cholesky solver\n";
    _solver.reset(new SparseCholeskyLinearSystemSolver());
  } else {
    _solver = _options.linearSystemSolver;
  }

  _options.verbose&& std::cout << "Using the " << _solver->name()
...
void Optimizer2::initialize() {
  ...
  SM_ASSERT_FALSE(Exception, _problem.get() == NULL,
                  "No optimization problem has been set");
  ...
  initEt.stop();
  SM_ASSERT_FALSE(Exception, _errorTerms.empty(),
                  "It is illegal to run the optimizer with no error terms.");
  Timer initMx("Optimizer2: Initialize---Matrices");
  // Set up the block matrix structure.
  // std::cout << "init structure\n";
  //      initializeLinearSolver();
  _solver->initMatrixStructure(_designVariables, _errorTerms,
                               _trustRegionPolicy->requiresAugmentedDiagonal());
  initMx.stop();
  _options.verbose&& std::cout
  ...
  _options.verbose&& std::cout << "The Jacobian matrix is " << dim << " x "
                               << columnBase << std::endl;
...
SolutionReturnValue Optimizer2::optimize() {
  ...
  while (srv.iterations < _options.maxIterations &&
         ...
         ((deltaX > _options.convergenceDeltaX &&
          ...
          linearSolverFailure)) {
    timeSolve.start();
    bool solutionSuccess = _trustRegionPolicy->solveSystem(
        _J, previousIterationFailed, _options.nThreads, _dx);
    timeSolve.stop();
...
DesignVariable* Optimizer2::designVariable(size_t i) {
  SM_ASSERT_LT_DBG(Exception, i, _designVariables.size(),
                   "index out of bounds");
...
void Optimizer2::computeDiagonalCovariances(SparseBlockMatrix& outP,
                                            double lambda) {
...
void Optimizer2::computeCovarianceBlocks(
    const std::vector<std::pair<int, int> >& /* blockIndices */,
    SparseBlockMatrix& /* outP */, double /* lambda */) {
  SM_THROW(Exception, "Broken");
}

void Optimizer2::computeCovariances(SparseBlockMatrix& /* outP */,
                                    double /* lambda */) {
  SM_THROW(Exception, "Broken");
}

void Optimizer2::computeHessian(SparseBlockMatrix& outH, double lambda) {
  std::shared_ptr<BlockCholeskyLinearSystemSolver> solver_sp;
  ...
  _options.verbose&& std::cout
      << "Setting the diagonal conditioner to: " << lambda << ".\n";
  evaluateError(false);
