...
namespace aslam {
namespace backend {
ErrorTerm::ErrorTerm() : _squaredError(0.0), _timestamp(0) {
  _mEstimatorPolicy = std::make_shared<NoMEstimator>();
}
...
///
/// the i/o variables outHessian and outRhs are the full Hessian and rhs in the
/// Gauss-Newton problem. The correct blocks for each design varible are
/// available from the design variable as dv.blockIndex()
void ErrorTerm::buildHessian(SparseBlockMatrix& outHessian,
...
void ErrorTerm::setMEstimatorPolicy(
    const std::shared_ptr<MEstimator>& mEstimator) {
...
DesignVariable* ErrorTerm::designVariable(size_t i) {
  SM_ASSERT_LT_DBG(aslam::IndexOutOfBoundsException, i, _designVariables.size(),
                   "index out of bounds");
...
void ErrorTerm::setDesignVariables(
    const std::vector<DesignVariable*>& designVariables) {
  ...
  SM_ASSERT_EQ(aslam::UnsupportedOperationException, _designVariables.size(), 0,
               "The design variable container already has objects. The design "
...
struct ErrorTermFunctor {
  ...
  Eigen::VectorXd operator()(const Eigen::VectorXd& dr) {
    ...
    for (size_t i = 0; i < _et.numDesignVariables(); i++) {
      ...
      SM_ASSERT_LE_DBG(aslam::Exception, offset + d->minimalDimensions(),
                       dr.size(), "The offset is out of bounds.");
      d->update(&dr[offset], d->minimalDimensions());
      offset += d->minimalDimensions();
    }
    SM_ASSERT_EQ_DBG(aslam::Exception, offset, dr.size(),
                     "The input vector is too large. It wasn't covered by the "
                     "design variables.");
    _et.evaluateError();
...
void ErrorTerm::evaluateJacobiansFiniteDifference(
