...
void SimpleOptimizationProblem::addDesignVariable(DesignVariable* dv,
  ...
  if (problemOwnsVariable)
    ...
    _designVariables.push_back(
        std::shared_ptr<DesignVariable>(dv, sm::null_deleter()));
}

/// \brief Add a design variable to the problem.
void SimpleOptimizationProblem::addDesignVariable(
...
void SimpleOptimizationProblem::addErrorTerm(ErrorTerm* ev,
                                             bool problemOwnsVariable) {
  if (problemOwnsVariable)
    addErrorTerm(std::shared_ptr<ErrorTerm>(ev));
  else
    addErrorTerm(std::shared_ptr<ErrorTerm>(ev, sm::null_deleter()));
}

/// \brief Add an error term to the problem
void SimpleOptimizationProblem::addErrorTerm(
    const std::shared_ptr<ErrorTerm>& et) {
  _errorTerms.push_back(et);
}

bool SimpleOptimizationProblem::isDesignVariableInProblem(
...
DesignVariable* SimpleOptimizationProblem::designVariableImplementation(
    size_t i) {
  return _designVariables[i].get();
}

const DesignVariable* SimpleOptimizationProblem::designVariableImplementation(
    size_t i) const {
  return _designVariables[i].get();
}
...
const ErrorTerm* SimpleOptimizationProblem::errorTermImplementation(
    size_t i) const {
  return _errorTerms[i].get();
}

void SimpleOptimizationProblem::getErrorsImplementation(
