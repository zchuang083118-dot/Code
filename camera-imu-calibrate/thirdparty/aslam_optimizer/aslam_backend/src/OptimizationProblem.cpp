...
void OptimizationProblem::addDesignVariable(DesignVariable* dv,
                                            bool problemOwnsVariable) {
  // \todo This is also bad and slow.
  for (unsigned i = 0; i < _designVariables.size(); ++i) {
    SM_ASSERT_NE(std::runtime_error, dv, _designVariables[i].get(),
                 "That design variable has already been added");
  }
  if (problemOwnsVariable)
    _designVariables.push_back(std::shared_ptr<DesignVariable>(dv));
  else
    _designVariables.push_back(
        std::shared_ptr<DesignVariable>(dv, [](DesignVariable*) {}));
}

/// \brief Add a design variable to the problem.
void OptimizationProblem::addDesignVariable(
    std::shared_ptr<DesignVariable> dv) {
  // \todo This is also bad and slow.
  for (unsigned i = 0; i < _designVariables.size(); ++i) {
...
void OptimizationProblem::addErrorTerm(ErrorTerm* ev,
                                       bool problemOwnsVariable) {
  if (problemOwnsVariable)
    addErrorTerm(std::shared_ptr<ErrorTerm>(ev));
  else
    addErrorTerm(std::shared_ptr<ErrorTerm>(ev, [](ErrorTerm*) {}));
}

/// \brief Add an error term to the problem
void OptimizationProblem::addErrorTerm(const std::shared_ptr<ErrorTerm>& et) {
  ...
  for (size_t i = 0; i < et->numDesignVariables(); ++i) {
    ...
    SM_ASSERT_TRUE_DBG(aslam::InvalidArgumentException,
                       isDesignVariableInProblem(dv),
                       "It is illegal to add an error term that contains a "
                       "missing design variable. Add the design variables to "
                       "the problem before adding the error terms.");
...
const DesignVariable* OptimizationProblem::designVariableImplementation(
    size_t i) const {
  return _designVariables[i].get();
}
...
void OptimizationProblem::getErrorsImplementation(
    const DesignVariable* dv, std::set<ErrorTerm*>& outErrorSet) {
  error_map_t::iterator it, it_end;
  // I'm not sure why this const cast is necessary but I get a failure on OSX if
  // it isn't here. Also, somehow this is painfully slow.
  std::tie(it, it_end) =
      _errorTermMap.equal_range(const_cast<DesignVariable*>(dv));
  for (; it != it_end; ++it) {
...
void OptimizationProblem::removeErrorTerm(const ErrorTerm* et) {
  ...
  std::vector<std::shared_ptr<ErrorTerm> >::iterator eit = _errorTerms.begin();
...
void OptimizationProblem::removeDesignVariable(const DesignVariable* dv) {
  ...
  std::vector<std::shared_ptr<DesignVariable> >::iterator dit =
      _designVariables.begin();
...
size_t OptimizationProblem::countActiveDesignVariables() {
  size_t c = 0;
  for (std::shared_ptr<DesignVariable> dv : _designVariables) {
    if (dv->isActive()) {
      c++;
    }
  }
