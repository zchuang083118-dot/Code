...
void marginalize(
    ...
    int numberOfInputDesignVariablesToRemove, bool useMEstimator,
    std::shared_ptr<aslam::backend::MarginalizationPriorErrorTerm>&
        outPriorErrorTermPtr,
  ...
  SM_WARN_STREAM_COND(inDesignVariables.size() == 0,
                      "Zero input design variables in the marginalizer!");

  // check for duplicates!
  std::unordered_set<aslam::backend::DesignVariable*> inDvSetHT;
  for (auto it = inDesignVariables.begin(); it != inDesignVariables.end();
       ++it) {
    auto ret = inDvSetHT.insert(*it);
    SM_ASSERT_TRUE(aslam::Exception, ret.second,
                   "Error! Duplicate design variables in input list!");
  }
  ...
  for (auto it = inErrorTerms.begin(); it != inErrorTerms.end(); ++it) {
    ...
    SM_ASSERT_TRUE(aslam::Exception, ret.second,
                   "Error! Duplicate error term in input list!");
  }
  SM_DEBUG_STREAM(
      "NO duplicates in input design variables or input error terms found.");
  ...
  for (std::vector<aslam::backend::DesignVariable*>::const_iterator it =
           inDesignVariables.begin();
  ...
  for (std::vector<aslam::backend::ErrorTerm*>::iterator it =
           inErrorTerms.begin();
       it != inErrorTerms.end(); ++it) {
  ...
  SM_INFO_STREAM("Marginalization optimization problem initialized with "
                 << inDesignVariables.size() << " design variables and "
                 << inErrorTerms.size() << " error terrms");
  ...
  SM_DEBUG_STREAM("Rank of jacobian: " << rank << " (full rank: " << fullRank
                                       << ", threshold: " << threshold << ")");
  bool rankDeficient = rank < fullRank;
  if (rankDeficient) {
    SM_WARN("Marginalization jacobian is rank deficient!");
  }
  // SM_ASSERT_FALSE(aslam::Exception, rankDeficient, "Right now, we don't want
  // the jacobian to be rank deficient - ever...");
  ...
  if (jrows < jcols) {
    ...
    R_reduced = jacobian.block(0, dimOfDesignVariablesToRemove, jrows,
                               jcols - dimOfDesignVariablesToRemove);
    d_reduced = b;

  } else {
    // PTF: Do we know what will happen when the jacobian matrix is rank
    // deficient? MB: yes, bad things!
    ...
    //			  size_t numRowsToKeep = rank -
    //dimOfDesignVariablesToRemove; 			  SM_ASSERT_TRUE_DBG(aslam::Exception,
    //rankDeficient || (numRowsToKeep == dimOfRemainingDesignVariables), "must
    //be the same if full rank!");

    // get the top left block
    SM_ASSERT_GE(aslam::Exception, R.rows(), numTopRowsInCov,
                 "Cannot extract " << numTopRowsInCov
                                   << " rows of R because it only has "
                                   << R.rows() << " rows.");
    SM_ASSERT_GE(aslam::Exception, R.cols(), numTopRowsInCov,
                 "Cannot extract " << numTopRowsInCov
                                   << " cols of R because it only has "
                                   << R.cols() << " cols.");
    // outRtop = R.block(0, 0, numTopRowsInRtop, numTopRowsInRtop);

    // cut off the zero rows at the bottom
    R_reduced =
        R.block(dimOfDesignVariablesToRemove, dimOfDesignVariablesToRemove,
                dimOfRemainingDesignVariables, dimOfRemainingDesignVariables);
    // R_reduced = R.block(dimOfDesignVariablesToRemove,
    // dimOfDesignVariablesToRemove, numRowsToKeep,
    // dimOfRemainingDesignVariables);

    d_reduced =
        d.segment(dimOfDesignVariablesToRemove, dimOfRemainingDesignVariables);
  ...
  std::shared_ptr<aslam::backend::MarginalizationPriorErrorTerm> err(
      new aslam::backend::MarginalizationPriorErrorTerm(
          remainingDesignVariables, d_reduced, R_reduced));
