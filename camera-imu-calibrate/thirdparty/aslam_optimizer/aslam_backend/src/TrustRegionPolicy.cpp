...
std::shared_ptr<LinearSystemSolver> TrustRegionPolicy::getSolver() {
  return _solver;
}

/// \brief set the linear system solver
void TrustRegionPolicy::setSolver(std::shared_ptr<LinearSystemSolver> solver) {
  _solver = solver;
}
...
bool TrustRegionPolicy::solveSystem(double J, bool previousIterationFailed,
  ...
  bool success =
      solveSystemImplementation(J, previousIterationFailed, nThreads, outDx);
