...
Optimizer::Optimizer(const OptimizerOptions& options) : _options(options) {
...
void Optimizer::setProblem(std::shared_ptr<OptimizationProblemBase> problem) {
  _problem = problem;
}
...
void Optimizer::initializeLinearSolver() {
  ...
  if (_options.linearSolver == "cholmod") {
    ...
    _solver.reset(
        new sparse_block_matrix::LinearSolverDense<Eigen::MatrixXd>());
  }
#ifndef QRSOLVER_DISABLED
    ...
    _options.verbose&& std::cout
        << "Unknown linear solver specified: " << _options.linearSolver
        << ". Using cholmod.\n";
    _solver.reset(
        new sparse_block_matrix::LinearSolverCholmod<Eigen::MatrixXd>());
  }
...
void Optimizer::initialize() {
  ...
  SM_ASSERT_FALSE(Exception, _designVariables.empty(),
                  "It is illegal to run the optimizer with all marginalized "
                  "design variables.");
  ...
  for (size_t i = 0; i < marginalizedDesignVariables.size(); ++i) {
    ...
    marginalizedDesignVariables[i]->setBlockIndex(i +
  ...
  // Surrently "blocks" just has the sizes of the minimal dimensions of each
  // block. the sparse matrix initialization requires the cumulative sum of
  // this.
  ...
  initMx.stop();
  _options.verbose&& std::cout
      << "Optimization problem initialized with " << _designVariables.size()
      << " design variables and " << _errorTerms.size() << " error terms\n";
  _options.verbose&& std::cout
      << "The dense part of the state is "
      << _H.rowBaseOfBlock(_marginalizedStartingBlock)
      << " parameters and the sparse part is "
      << _H.cols() - _H.rowBaseOfBlock(_marginalizedStartingBlock)
      << " parameters\n";
}
  ...
  if (_options.levenbergMarquardtEstimateLambdaScale <= 0) {
    ...
    for (int i = 0; i < _H.bRows(); i++) {
      const Eigen::MatrixXd* H =
          _H.block(i, i, false);  // do no allocate if missing!
      if (H) {
    ...
    _options.verbose&& std::cout << "Initialised Lambda: " << _lambda
                                 << std::endl;
  }
}
...
SolutionReturnValue Optimizer::optimizeDogLeg() {
  ...
  // Select the design variables and (eventually) the error terms involved in
  // the optimization.
  initialize();
  ...
  // *** while not done
  _options.verbose&& std::cout << "[" << srv.iterations << ".0]: J: " << _J
                               << std::endl;
  ...
  // choose initial delta to be something between GN and SD but biased towards
  // GN would be ideal but to avoid computing the GN solution in the first step
  // before looping we take the SD norm.
  double _delta = 0;
  double _p_delta = 0;
  ...
  // while( !evaluateStoppingCriterion() )
  {
    ...
    do {
      ...
      if (_dx_sd_norm >= _delta && _delta != 0) {
        _dx = _delta / _dx_sd_norm *
              _dx_sd;  // scale SD step to fit into trust region
      ...
      // otherwise check the GN step
        ...
        if (!gnComputed) {
          ...
          timeSchur.start();
          applySchurComplement(_H, _rhs, _lambda, _marginalizedStartingBlock,
                               _options.doLevenbergMarquardt, _A, _invVi, _b);
          timeSchur.stop();
          ...
          if (!solutionSuccess) {
            // the default solver failed. try the QR solver as a robust sparse
            // alternative:
            if (!_fallbackSolver) {  // initialise a new solver
              // for now take QR
              /// \todo: add an intelligent fallbackSolver structure.
              /// maybe add it as a property of the linear solvers and directly
              /// name the solvers in their class instead of the optimizer
              /// init()
              _fallbackSolver.reset(
                  new sparse_block_matrix::LinearSolverQr<Eigen::MatrixXd>());
              _fallbackSolver->init();
              _options.verbose&& std::cout
                  << "Optimizer: The linear solution failed. Fallback to QR. ("
            ...
            if (!solutionSuccess) {
              _options.verbose&& std::cout
                  << "Optimizer: The linear solution failed. Again... ("
                  << linearSolverFailCounter << ")" << std::endl;
              _dx_gn = _dx_sd;
            }
            ...
            // SM_ASSERT_TRUE(Exception, linearSolverFailCounter <=
            // _options.linearSolverMaximumFails, "The linear solution failed");
          }
        ...
        if (_dx_gn_norm <= _delta) {
          ...
          L0 = _J;
          stepType = "GN";
        } else  // otherwise interpolate on the line between the cauchy point
                // and gn step
        {
          // get beta:
          ...
          if (c <= 0) {
            beta =
                -c + sqrt(c * c + gdnsd_norm_sqr * (_delta * _delta -
                                                    _dx_sd_norm * _dx_sd_norm));
            beta /= gdnsd_norm_sqr;
          } else {
          ...
          L0 =
              1 / 2 * _sd_scale * (1 - beta) * (1 - beta) * _rhs.squaredNorm() +
              beta * (2 - beta) * _J;
          stepType = "DL";
        }
      }
    ...
    _options.verbose&& std::cout << "[" << srv.iterations << "]: J: " << _J
                                 << ", dJ: " << deltaJ << ", deltaX: " << deltaX
...
SolutionReturnValue Optimizer::optimize() {
  ...
  while (srv.iterations < _options.maxIterations &&
    ...
    if (!solutionSuccess) {
      ...
      // SM_ASSERT_TRUE(Exception, solutionSuccess, "The linear solution
      // failed");
      isLmRegression = true;
...
DesignVariable* Optimizer::denseVariable(size_t i) {
  SM_ASSERT_LT_DBG(Exception, (int)i, _marginalizedStartingBlock,
                   "index out of bounds");
  return _designVariables[i];
}

DesignVariable* Optimizer::sparseVariable(size_t i) {
  SM_ASSERT_LT_DBG(Exception, i + _marginalizedStartingBlock,
...
void Optimizer::computeCovarianceBlocks(
    const std::vector<std::pair<int, int> >& blockIndices) {
  /// \todo Figure out properly why I have to create a new linear solver here.
  std::shared_ptr<sparse_block_matrix::LinearSolverCholmod<Eigen::MatrixXd> >
      solver(new sparse_block_matrix::LinearSolverCholmod<Eigen::MatrixXd>());
...
const Eigen::MatrixXd* Optimizer::getCovarianceBlock(int blockRow,
                                                     int blockCol) const {
