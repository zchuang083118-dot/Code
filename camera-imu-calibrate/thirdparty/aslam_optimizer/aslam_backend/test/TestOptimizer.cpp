...
TEST(OptimizerTestSuite, testOptimizerMatrices) {
  try {
    ...
    std::shared_ptr<OptimizationProblem> problem_ptr(new OptimizationProblem);
    OptimizationProblem& problem = *problem_ptr;
    const int P = 3;
    const int E = 2;
    // Add some design variables.
    std::vector<std::shared_ptr<Point2d> > p2d;
    for (int p = 0; p < P; ++p) {
      std::shared_ptr<Point2d> point(new Point2d(Eigen::Vector2d::Random()));
      p2d.push_back(point);
    ...
    std::vector<std::shared_ptr<LinearErr> > e1;
    for (int p = 0; p < P; ++p) {
      for (int e = 0; e < E; ++e) {
        std::shared_ptr<LinearErr> err(new LinearErr(p2d[p].get()));
        e1.push_back(err);
    ...
    // std::vector< std::shared_ptr<LinearErr2> > e2;
    // Point2d * ps = p2d[P-1].get();
    // ps->setMarginalized(true);
    // for(int p = 0; p < P-1; ++p)
    //   {
    //  std::shared_ptr<LinearErr2> err(new LinearErr2(p2d[p].get(), ps) )
    //  e2.push_back( err );
    //  problem.addErrorTerm(err);
    //  ErrorTermTestHarness<2> eh(err.get());
    //  eh.testAll();
    ...
    //   {
    //  std::cout << "dv[" << p << "] index: " <<
    //  problem.designVariable(p).blockIndex() << std::endl;
    //   }
    // for(unsigned i = 0; i < problem.numErrorTerms(); ++i)
    //   {
    //  std::cout << "e[" << i << "] has " <<
    //  problem.errorTerm(i).numDesignVariables() << " design variables\n";
    //  for(unsigned j = 0; j < problem.errorTerm(i).numDesignVariables(); ++j)
    //    {
    //      std::cout << "\tdv[" << j << "] has block index " <<
    //      problem.errorTerm(i).designVariable(j)->blockIndex() << std::endl;
    //      std::cout << "\tdv[" << j << "]: " << (
    ...
    rhs.setZero();
    Eigen::MatrixXd invR(problem.numErrorTerms() * 2,
                         problem.numErrorTerms() * 2);
    invR.setZero();
    std::shared_ptr<SparseBlockMatrix> cloneH(optimizer.H().clone());
    cloneH->clear();
    ...
    for (unsigned i = 0; i < problem.numErrorTerms(); ++i) {
      ...
      J.block<2, P * 2>(ridx, 0) =
          jc.asDenseMatrix(optimizer.H().colBlockIndices());
    ...
    sm::eigen::assertNear(estH, H, 1e-14, SM_SOURCE_FILE_POS,
                          "Did we build the Hessian correctly? A is from J^T * "
                          "J and B is from the optimizer.");
    ...
    for (unsigned i = 0; i < problem.numErrorTerms(); ++i) {
      ...
      e.segment<2>(ridx) =
          dynamic_cast<ErrorTermFs<2>*>(problem.errorTerm(i))->error();
...
TEST(OptimizerTestSuite, testOptimizerNormalized) {
  try {
    ...
    for (int p = 0; p < P; ++p) {
      ...
      std::shared_ptr<Point2d> point(new Point2d(pts));
    ...
    optimizer.setProblem(problem_ptr);
    optimizer.optimize();
    //    std::cout << "A:" << std::endl << optimizer.H().toDense() <<
    //    std::endl; std::cout << "b:" << std::endl << optimizer.rhs() <<
    //    std::endl;
    //        std::shared_ptr<OptimizationProblem> problem_ptr2( new
    //        OptimizationProblem ); OptimizationProblem & problem2 =
    //        *problem_ptr2;
    //
    //        // reset the points:
    //
    ...
    //          p2d[p]->setScaling(1);
    //          std::cout << "scale [" << p << "] " << p2d[p]->scaling() <<
    //          std::endl; problem2.addDesignVariable(p2d[p]);
    //
    //
    //
    //
    ...
    //        for(int p = 0; p < P; ++p)
    //          {
    //          std::shared_ptr<Point2d> point( new Point2d( points[p] ) );
    //          p2d.push_back( point );
    ...
    //        optimizer2.optimize();
    //   std::cout << "H2: " << std::endl << optimizer2.H().toDense() <<
    //   std::endl; std::cout << "H1: " << std::endl << optimizer.H().toDense()
    //   << std::endl; std::cout << "rhs2" << std::endl << optimizer2.rhs() <<
    //   std::endl; std::cout << "rhs1" << std::endl << optimizer.rhs() <<
    //   std::endl;
    // verify
    // sm::eigen::assertNear(optimizer.J(),optimizer2.J(), 1e-3,
    // SM_SOURCE_FILE_POS);
  } catch (const std::exception& e) {
    FAIL() << e.what();
  }
