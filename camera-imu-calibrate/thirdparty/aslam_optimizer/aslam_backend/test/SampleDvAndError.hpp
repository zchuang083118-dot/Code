...
class LinearErr : public aslam::backend::ErrorTermFs<2> {
 public:
  ...
  virtual void evaluateJacobiansImplementation(
      aslam::backend::JacobianContainer& outJ) const {
    outJ.add(_p2d, -_J);
  }
};
...
class LinearErr2 : public aslam::backend::ErrorTermFs<2> {
 public:
  ...
  LinearErr2(Point2d* p2d1, Point2d* p2d2) : _p2d1(p2d1), _p2d2(p2d2) {
    ...
    _J1.setRandom();
    _J2.setRandom();
    parent_t::setDesignVariables((aslam::backend::DesignVariable*)_p2d1,
                                 (aslam::backend::DesignVariable*)_p2d2);
    setInvR(sm::eigen::randomCovariance<2>());
...
class LinearErr3 : public aslam::backend::ErrorTermFs<4> {
 public:
  ...
  LinearErr3(Point2d* p2d1, Point2d* p2d2, Point2d* p3)
      : _p2d1(p2d1), _p2d2(p2d2), _p3(p3) {
    _p2d1->setActive(true);
    _p2d2->setActive(true);
    _p3->setActive(true);
    _J1.setRandom();
...
inline void buildSystem(int D, int E,
  ...
  for (int i = 0; i < E; ++i) {
    ...
    if (mod == 0) {
      ...
      // std::cout << "le2\n";
      errs.push_back(new LinearErr2((Point2d*)dvs[i % dvs.size()],
                                    (Point2d*)dvs[(i + 1) % dvs.size()]));
    } else {
      // std::cout << "le3\n";
      errs.push_back(new LinearErr3((Point2d*)dvs[i % dvs.size()],
                                    (Point2d*)dvs[(i + 1) % dvs.size()],
...
inline std::shared_ptr<aslam::backend::OptimizationProblem> buildProblem(
    int seed, int D, int E) {
  using namespace aslam::backend;
  srand(seed);
  sm::random::seed(seed);
  std::vector<aslam::backend::DesignVariable*> dvs;
  ...
  std::shared_ptr<OptimizationProblem> problem(new OptimizationProblem);
  for (size_t i = 0; i < dvs.size(); ++i) {
