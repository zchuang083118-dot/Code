...
TEST(SparseMatrixFunctionTests, testSchurComplement1) {
  try {
    ...
    b.setZero();
    std::shared_ptr<SparseBlockMatrix> A(H.slice(
        0, marginalizedStartingBlock, 0, marginalizedStartingBlock, true));
    ...
    *H.block(4, 4, true) = sm::eigen::randomCovariance<3>() * 100;
    ...
    // std::cout << "H:\n" << H << std::endl;
    applySchurComplement(H, e, lambda, marginalizedStartingBlock, true, *A,
                         invVi, b);
    Eigen::MatrixXd Hd = H.toDense();
    Eigen::MatrixXd U =
        Hd.topLeftCorner(marginalizedStartingIndex, marginalizedStartingIndex);
    Eigen::MatrixXd V =
        Hd.bottomRightCorner(H.rows() - marginalizedStartingIndex,
                             H.rows() - marginalizedStartingIndex);
    Eigen::MatrixXd W = Hd.topRightCorner(marginalizedStartingIndex,
                                          H.rows() - marginalizedStartingIndex);
    ...
    Eigen::VectorXd bb = epsDense - W * V.inverse() * epsSparse;
    {
      ...
      sm::eigen::assertNear(Asa, AA, 1e-12, SM_SOURCE_FILE_POS,
                            "Testing the lhs schur complement");
    }
    {
      SCOPED_TRACE("");
      sm::eigen::assertNear(b, bb, 1e-12, SM_SOURCE_FILE_POS,
                            "Testing the rhs schur complement");
    }
    ...
    for (int i = 0; i < H.bRows() - marginalizedStartingBlock; ++i) {
      ...
      Eigen::VectorXd dsi =
          denseDs.segment(H.rowBaseOfBlock(i + marginalizedStartingBlock) -
                              marginalizedStartingIndex,
                          H.rowsOfBlock(i + marginalizedStartingBlock));
      sm::eigen::assertNear(outDsi, dsi, 1e-12, SM_SOURCE_FILE_POS,
                            "Checking the update step calculation");
    }
    ...
    for (int i = 0; i < H.bRows() - marginalizedStartingBlock; ++i) {
      ...
      Eigen::VectorXd dsi =
          denseDs.segment(H.rowBaseOfBlock(i + marginalizedStartingBlock) -
                              marginalizedStartingIndex,
                          H.rowsOfBlock(i + marginalizedStartingBlock));
      sm::eigen::assertNear(outDsi, dsi, 1e-14, SM_SOURCE_FILE_POS,
                            "Checking the update step calculation");
    }
  } catch (const std::exception& e) {
    FAIL() << "Exception: " << e.what();
