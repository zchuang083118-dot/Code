...
struct JacobianColumnPointer {
  JacobianColumnPointer(size_t sc = 0, size_t epc = 0)
      : startValueIndex(sc), elementsPerColumn(epc) {}
...
class CompressedColumnMatrix : public Matrix {
 public:
  ...
  /// \brief get the element at row r and column c. r and c are checked for
  /// validity
   ...
   *
   * @param Jrows The number of rows in the Jacobian (corresponding to the
   * number of elements in an error term)
   * @param dvs The list of design variables with non-zero Jacobians in this
   * column.
   * @return outColumnPointer A pointer to the value array of the Jacobian
   * matrix.
   */
  JacobianColumnPointer appendJacobiansSymbolic(
      int Jrows, const std::vector<DesignVariable*>& dvs);

  /// \brief Write the Jacobian values to the matrix using the pointer provided
  /// by appendJacobiansSymbolic()
  void writeJacobians(const JacobianContainer& jc,
                      const JacobianColumnPointer& cp);

  /// \brief A convenience function that calls appendJacobiansSymbolic() and
  /// then writeJacobians()
  ...
  ///        The diagonal vector must have the same number of rows as the
  ///        matrix.
