...
class OptimizationProblem : public OptimizationProblemBase {
 public:
  ...
  void addDesignVariable(std::shared_ptr<DesignVariable> dv);

  /// \brief Remove the design variable. This may cause error terms to also be
  /// removed.
  ...
  virtual void addErrorTerm(const std::shared_ptr<ErrorTerm>& et);
 ...
 protected:
  ...
  // \todo Replace these std::vectors by something better. The underlying
  // algorithms that this object
  //       supports suck with these containers. Blerg. See
  //       "removeDesignVariable()" for an example of just how bad this is.
  std::vector<std::shared_ptr<DesignVariable> > _designVariables;
  std::vector<std::shared_ptr<ErrorTerm> > _errorTerms;
