 ...
 * \class ErrorTerm
 *
 * \brief a class representing a single error term in a nonlinear least squares
 * problem.
 *
 * The ErrorTerm class is an abstract base class that is able to handle both
 * "normal" vector valued error terms, as one would find in a nonlinear least
 * squares problem, and "quadratic terms", the smoothing matrices found in
 * continuous-time estimation problems.
 *
 * For normal error terms of dimension D, child classes should derive from
 * ErrorTermFS<D>.
 */
class ErrorTerm {
 public:
  ...
  typedef std::shared_ptr<aslam::backend::ErrorTerm> Ptr;

  ErrorTerm();
  virtual ~ErrorTerm();

  /// \brief evaluate the error term.
  ///        After this is called, the _squaredError is filled in with \f$
  ///        \mathbf e^T \mathbf R^{-1} \mathbf e \f$
  ...
  virtual void getWeightedJacobians(JacobianContainer& outJc,
                                    bool useMEstimator) = 0;

  virtual void getWeightedError(Eigen::VectorXd& e,
                                bool useMEstimator) const = 0;

  /// \brief get the current value of the error.
  /// This was put here to make the python interface easier to generate. It
  /// doesn't fit for quadratic integral terms so it may go away in future
  /// versions.
  ...
  void setMEstimatorPolicy(const std::shared_ptr<MEstimator>& mEstimator);
  ...
  ///
  /// the i/o variables outHessian and outRhs are the full Hessian and rhs in
  /// the Gauss-Newton problem. The correct blocks for each design varible are
  /// available from the design variable as dv.blockIndex()
  void buildHessian(SparseBlockMatrix& outHessian, Eigen::VectorXd& outRhs,
                    bool useMEstimator);
 ...
 protected:
  ...
  /// \brief child classes should set the set of design variables usign this
  /// function.
 ...
 *
 * This class fills in some of the functionality needed for "normal",
 * vector-valued error terms. In most cases, one will want to derive from this
 * class when implementing an error term.
 *
 */
