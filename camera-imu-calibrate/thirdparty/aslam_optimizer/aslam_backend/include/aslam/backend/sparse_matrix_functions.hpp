...
void applySchurComplement(
    ...
    const Eigen::VectorXd& e, double lambda, int marginalizedStartingBlock,
    bool doLevenberg,
    sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd>& A,
    std::vector<Eigen::MatrixXd>& invVi, Eigen::VectorXd& b);

void buildDsi(int i, sparse_block_matrix::SparseBlockMatrix<Eigen::MatrixXd>& H,
              const Eigen::VectorXd& e, int marginalizedStartingBlock,
              const Eigen::MatrixXd& invVi, const Eigen::VectorXd& dx,
