...
class MarginalizationPriorErrorTerm : public aslam::backend::ErrorTermDs {
 public:
  ...
  typedef std::shared_ptr<aslam::backend::MarginalizationPriorErrorTerm> Ptr;

  // creates the marginalization error term of the form e(x) = d - R*x
  // designVariables: 	the design variables of this marginalization prior error
  // term
 ...
 private:
  ...
  // computes the minimal difference of all design variables between the
  // linearization point at marginalization and the current guess (i.e.
  // log(x_bar - x))
