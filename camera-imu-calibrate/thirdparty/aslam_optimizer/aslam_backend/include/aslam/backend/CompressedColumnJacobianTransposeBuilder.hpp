...
class CompressedColumnJacobianTransposeBuilder {
 public:
  ...
  ///
  /// This function assumes that all the design variables in the DV container
  /// are active and that the container is sorted by order of block index such
  /// that dv[i]->blockIndex() == i
  ///
  virtual void initMatrixStructure(const std::vector<DesignVariable*>& dvs,
                                   const std::vector<ErrorTerm*>& errors);

  /// \brief build the large, sparse internal Jacobian matrix from the error
  /// terms.
  virtual void buildSystem(size_t nThreads, bool useMEstimator);

  /// \brief Get a view of the transpose of the Jacobian as a cholmod sparse
  /// matrix.
 ...
 private:
  ...
  void evaluateJacobians(int threadId, int startIdx, int endIdx,
                         bool useMEstimator);
  ...
  std::shared_ptr<cholmod_sparse> _J;
  ...
  /// \brief An array parallel to the error term array that maps error terms to
  /// parts of the Jacobian.
  ...
  void setupThreadedJob(MEMBER_FUNCTION_PTR ptr, size_t nThreads,
                        bool useMEstimator);
};
}  // namespace backend
}  // namespace aslam
