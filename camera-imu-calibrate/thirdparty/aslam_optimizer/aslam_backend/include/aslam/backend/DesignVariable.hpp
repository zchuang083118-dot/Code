...
class DesignVariable {
 public:
   ...
   */
  struct BlockIndexOrdering {
    inline bool operator()(const DesignVariable* lhs,
                           const DesignVariable* rhs) const {
  ...
  typedef std::shared_ptr<DesignVariable> Ptr;

  /**
   * \typedef blockordered_set_t
   * \brief a set of design variables ordered by block index.
   */
  //      typedef std::unordered_set//set<DesignVariable *, BlockIndexOrdering>
  //      blockordered_set_t;
  ...
  /// \brief get the block index used in the optimization routine. -1 if not
  /// being optimized.
  ...
  /// \brief Computes the minimal distance in tangent space between the current
  /// value of the DV and xHat
  void minimalDifference(const Eigen::MatrixXd& xHat,
                         Eigen::VectorXd& outDifference) const;

  /// \brief Computes the minimal distance in tangent space between the current
  /// value of the DV and xHat and the jacobian
  void minimalDifferenceAndJacobian(const Eigen::MatrixXd& xHat,
 ...
 protected:
  ...
  /// Computes the minimal distance in tangent space between the current value
  /// of the DV and xHat
  virtual void minimalDifferenceImplementation(
      const Eigen::MatrixXd& xHat, Eigen::VectorXd& outDifference) const;

  /// Computes the minimal distance in tangent space between the current value
  /// of the DV and xHat and the jacobian
  virtual void minimalDifferenceAndJacobianImplementation(
      const Eigen::MatrixXd& xHat, Eigen::VectorXd& outDifference,
      Eigen::MatrixXd& outJacobian) const;
