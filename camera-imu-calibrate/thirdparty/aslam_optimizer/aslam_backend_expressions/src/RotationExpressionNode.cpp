...
Eigen::Matrix3d RotationExpressionNode::toRotationMatrix() const {
  return toRotationMatrixImplementation();
}

void RotationExpressionNode::evaluateJacobians(
...
void RotationExpressionNode::getDesignVariables(
...
RotationExpressionNodeMultiply::RotationExpressionNodeMultiply(
    ...
    std::shared_ptr<RotationExpressionNode> rhs)
    : _lhs(lhs), _rhs(rhs) {
...
void RotationExpressionNodeMultiply::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
...
void RotationExpressionNodeMultiply::getDesignVariablesImplementation(
    DesignVariable::set_t& designVariables) const {
...
RotationExpressionNodeInverse::RotationExpressionNodeInverse(
    std::shared_ptr<RotationExpressionNode> dvRotation)
...
void RotationExpressionNodeInverse::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
...
void RotationExpressionNodeInverse::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians,
    const Eigen::MatrixXd& applyChainRule) const {
  _dvRotation->evaluateJacobians(outJacobians,
                                 -applyChainRule * _C.transpose());
}

void RotationExpressionNodeInverse::getDesignVariablesImplementation(
    DesignVariable::set_t& designVariables) const {
  _dvRotation->getDesignVariables(designVariables);
}

RotationExpressionNodeTransformation::RotationExpressionNodeTransformation(
    std::shared_ptr<TransformationExpressionNode> transformation)
    : _transformation(transformation) {}

RotationExpressionNodeTransformation::~RotationExpressionNodeTransformation() {}

Eigen::Matrix3d
RotationExpressionNodeTransformation::toRotationMatrixImplementation() const {
