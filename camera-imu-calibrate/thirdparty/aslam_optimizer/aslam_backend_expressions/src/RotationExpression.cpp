...
RotationExpression::RotationExpression(
    RotationExpressionNode* rotationDesignVariable) {
  _root.reset(rotationDesignVariable, sm::null_deleter());
  SM_ASSERT_TRUE(
      Exception, _root.get() != NULL,
...
RotationExpression::RotationExpression(
    std::shared_ptr<RotationExpressionNode> node)
    : _root(node) {
  SM_ASSERT_TRUE(
      Exception, _root.get() != NULL,
      "It is illegal to initialized a rotation expression with a null node");
...
RotationExpression RotationExpression::inverse() {
  std::shared_ptr<RotationExpressionNode> newRoot(
      new RotationExpressionNodeInverse(_root));
  return RotationExpression(newRoot);
}

/// \brief Evaluate the Jacobians
void RotationExpression::evaluateJacobians(
    JacobianContainer& outJacobians) const {
  _root->evaluateJacobians(outJacobians);
}

RotationExpression RotationExpression::operator*(
    const RotationExpression& p) const {
  std::shared_ptr<RotationExpressionNode> newRoot(
      new RotationExpressionNodeMultiply(_root, p._root));
  return RotationExpression(newRoot);
}

EuclideanExpression RotationExpression::operator*(
    const EuclideanExpression& p) const {
  std::shared_ptr<EuclideanExpressionNode> newRoot(
      new EuclideanExpressionNodeMultiply(_root, p._root));
  return EuclideanExpression(newRoot);
}

HomogeneousExpression RotationExpression::operator*(
    const HomogeneousExpression& /* p */) const {
  // \todo
