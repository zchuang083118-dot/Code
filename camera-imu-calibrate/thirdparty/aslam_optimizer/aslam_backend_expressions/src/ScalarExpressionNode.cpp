...
void ScalarExpressionNode::evaluateJacobians(
...
void ScalarExpressionNode::evaluateJacobians(
  ...
  SM_ASSERT_EQ_DBG(Exception, applyChainRule.cols(), 1,
                   "The chain rule matrix must have one columns");
  evaluateJacobiansImplementation(outJacobians, applyChainRule);
}

void ScalarExpressionNode::getDesignVariables(
...
ScalarExpressionNodeMultiply::ScalarExpressionNodeMultiply(
    std::shared_ptr<ScalarExpressionNode> lhs,
    std::shared_ptr<ScalarExpressionNode> rhs)
    : _lhs(lhs), _rhs(rhs) {}
...
void ScalarExpressionNodeMultiply::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
...
void ScalarExpressionNodeMultiply::getDesignVariablesImplementation(
    DesignVariable::set_t& designVariables) const {
...
void ScalarExpressionNodeDivide::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
  Eigen::MatrixXd L(1, 1), R(1, 1);
...
void ScalarExpressionNodeDivide::getDesignVariablesImplementation(
    DesignVariable::set_t& designVariables) const {
  _lhs->getDesignVariables(designVariables);
  _rhs->getDesignVariables(designVariables);
}
...
ScalarExpressionNodeAdd::ScalarExpressionNodeAdd(
    std::shared_ptr<ScalarExpressionNode> lhs,
    std::shared_ptr<ScalarExpressionNode> rhs, double multiplyRhs)
...
void ScalarExpressionNodeAdd::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
...
void ScalarExpressionNodeAdd::getDesignVariablesImplementation(
    DesignVariable::set_t& designVariables) const {
  _lhs->getDesignVariables(designVariables);
  _rhs->getDesignVariables(designVariables);
}
...
ScalarExpressionNodeFromVectorExpression::
    ScalarExpressionNodeFromVectorExpression(
        std::shared_ptr<VectorExpressionNode<1> > lhs)
    : _lhs(lhs) {}

ScalarExpressionNodeFromVectorExpression::
    ~ScalarExpressionNodeFromVectorExpression() {}

double ScalarExpressionNodeFromVectorExpression::toScalarImplementation()
    const {
