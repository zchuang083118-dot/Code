...
void EuclideanExpressionNode::evaluateJacobians(
...
void EuclideanExpressionNode::evaluateJacobians(
  ...
  SM_ASSERT_EQ_DBG(Exception, applyChainRule.cols(), 3,
                   "The chain rule matrix must have three columns");
  evaluateJacobiansImplementation(outJacobians, applyChainRule);
}

void EuclideanExpressionNode::getDesignVariables(
...
EuclideanExpressionNodeMultiply::EuclideanExpressionNodeMultiply(
    std::shared_ptr<RotationExpressionNode> lhs,
    std::shared_ptr<EuclideanExpressionNode> rhs)
    : _lhs(lhs), _rhs(rhs) {
...
void EuclideanExpressionNodeMultiply::getDesignVariablesImplementation(
    DesignVariable::set_t& designVariables) const {
...
void EuclideanExpressionNodeMultiply::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
  _lhs->evaluateJacobians(outJacobians,
                          sm::kinematics::crossMx(_C_lhs * _p_rhs));
...
void EuclideanExpressionNodeMultiply::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians,
    const Eigen::MatrixXd& applyChainRule) const {
  _lhs->evaluateJacobians(
      outJacobians, applyChainRule * sm::kinematics::crossMx(_C_lhs * _p_rhs));
...
EuclideanExpressionNodeMatrixMultiply::EuclideanExpressionNodeMatrixMultiply(
    std::shared_ptr<MatrixExpressionNode> lhs,
    std::shared_ptr<EuclideanExpressionNode> rhs)
    : _lhs(lhs), _rhs(rhs) {
...
EuclideanExpressionNodeMatrixMultiply::
    ~EuclideanExpressionNodeMatrixMultiply() {}
...
Eigen::Vector3d
EuclideanExpressionNodeMatrixMultiply::toEuclideanImplementation() const {
  _A_lhs = _lhs->toMatrix3x3();
  _p_rhs = _rhs->toEuclidean();
...
void EuclideanExpressionNodeMatrixMultiply::evaluateJacobiansImplementation(
  ...
  J_full << _p_rhs(0) * Eigen::Matrix3d::Identity(),
      _p_rhs(1) * Eigen::Matrix3d::Identity(),
...
EuclideanExpressionNodeCrossEuclidean::EuclideanExpressionNodeCrossEuclidean(
    std::shared_ptr<EuclideanExpressionNode> lhs,
    std::shared_ptr<EuclideanExpressionNode> rhs)
    : _lhs(lhs), _rhs(rhs) {}
...
Eigen::Vector3d
EuclideanExpressionNodeCrossEuclidean::toEuclideanImplementation() const {
...
void EuclideanExpressionNodeCrossEuclidean::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
  _lhs->evaluateJacobians(outJacobians,
                          -sm::kinematics::crossMx(_rhs->toEuclidean()));
  _rhs->evaluateJacobians(outJacobians,
                          sm::kinematics::crossMx(_lhs->toEuclidean()));
}
...
void EuclideanExpressionNodeCrossEuclidean::evaluateJacobiansImplementation(
  ...
  _lhs->evaluateJacobians(
      outJacobians,
      -applyChainRule * sm::kinematics::crossMx(_rhs->toEuclidean()));
  _rhs->evaluateJacobians(
      outJacobians,
      applyChainRule * sm::kinematics::crossMx(_lhs->toEuclidean()));
}
...
void EuclideanExpressionNodeAddEuclidean::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
  _lhs->evaluateJacobians(outJacobians);
  _rhs->evaluateJacobians(outJacobians);
}
...
Eigen::Vector3d
EuclideanExpressionNodeSubtractEuclidean::toEuclideanImplementation() const {
  return _lhs->toEuclidean() - _rhs->toEuclidean();
}
...
EuclideanExpressionNodeSubtractVector::EuclideanExpressionNodeSubtractVector(
    std::shared_ptr<EuclideanExpressionNode> lhs, const Eigen::Vector3d& rhs)
    : _lhs(lhs), _rhs(rhs) {}

EuclideanExpressionNodeSubtractVector::
    ~EuclideanExpressionNodeSubtractVector() {}
...
Eigen::Vector3d
EuclideanExpressionNodeSubtractVector::toEuclideanImplementation() const {
...
EuclideanExpressionNodeNegated::EuclideanExpressionNodeNegated(
    std::shared_ptr<EuclideanExpressionNode> operand)
    : _operand(operand) {}

EuclideanExpressionNodeNegated::~EuclideanExpressionNodeNegated() {}

void EuclideanExpressionNodeNegated::getDesignVariablesImplementation(
    DesignVariable::set_t& designVariables) const {
...
void EuclideanExpressionNodeNegated::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
...
EuclideanExpressionNodeScalarMultiply::EuclideanExpressionNodeScalarMultiply(
    ...
    std::shared_ptr<ScalarExpressionNode> s)
    : _p(p), _s(s) {}
...
Eigen::Vector3d
EuclideanExpressionNodeScalarMultiply::toEuclideanImplementation() const {
  return _p->toEuclidean() * _s->toScalar();
}

void EuclideanExpressionNodeScalarMultiply::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
  _p->evaluateJacobians(outJacobians,
                        Eigen::Matrix3d::Identity() * _s->toScalar());
...
VectorExpression2EuclideanExpressionAdapter::
    VectorExpression2EuclideanExpressionAdapter(
        std::shared_ptr<VectorExpressionNode<3> > vectorExpressionNode)
    : _vectorExpressionNode(vectorExpressionNode) {}

VectorExpression2EuclideanExpressionAdapter::
    ~VectorExpression2EuclideanExpressionAdapter() {}

void VectorExpression2EuclideanExpressionAdapter::
...
Eigen::Vector3d
VectorExpression2EuclideanExpressionAdapter::toEuclideanImplementation() const {
  return _vectorExpressionNode->toVector();
}

void VectorExpression2EuclideanExpressionAdapter::
    evaluateJacobiansImplementation(JacobianContainer& outJacobians) const {
  _vectorExpressionNode->evaluateJacobians(outJacobians,
...
void VectorExpression2EuclideanExpressionAdapter::
    evaluateJacobiansImplementation(
        JacobianContainer& outJacobians,
        const Eigen::MatrixXd& applyChainRule) const {
  _vectorExpressionNode->evaluateJacobians(
      outJacobians, applyChainRule * Eigen::Matrix3d::Identity());
}

EuclideanExpressionNodeTranslation::EuclideanExpressionNodeTranslation(
    std::shared_ptr<TransformationExpressionNode> operand)
    : _operand(operand) {}
...
void EuclideanExpressionNodeTranslation::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
...
EuclideanExpressionNodeRotationParameters::
    EuclideanExpressionNodeRotationParameters(
        std::shared_ptr<RotationExpressionNode> operand,
...
Eigen::Vector3d
EuclideanExpressionNodeRotationParameters::toEuclideanImplementation() const {
...
void EuclideanExpressionNodeRotationParameters::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
  Eigen::MatrixXd J =
      _rk->parametersToSMatrix(
             _rk->rotationMatrixToParameters(_operand->toRotationMatrix()))
          .inverse();
  _operand->evaluateJacobians(outJacobians, J);
}
...
void EuclideanExpressionNodeRotationParameters::
    getDesignVariablesImplementation(
        DesignVariable::set_t& designVariables) const {
  return _operand->getDesignVariables(designVariables);
}
...
EuclideanExpressionNodeFromHomogeneous::EuclideanExpressionNodeFromHomogeneous(
    std::shared_ptr<HomogeneousExpressionNode> root)
    : _root(root) {}
EuclideanExpressionNodeFromHomogeneous::
    ~EuclideanExpressionNodeFromHomogeneous() {}

Eigen::Vector3d
EuclideanExpressionNodeFromHomogeneous::toEuclideanImplementation() const {
...
void EuclideanExpressionNodeFromHomogeneous::evaluateJacobiansImplementation(
  ...
  // Eigen::Matrix<double,3,4> * jacobian = NULL);
...
EuclideanExpressionNodeElementwiseMultiplyEuclidean::
    EuclideanExpressionNodeElementwiseMultiplyEuclidean(
        std::shared_ptr<EuclideanExpressionNode> lhs,
        std::shared_ptr<EuclideanExpressionNode> rhs)
    : _lhs(lhs), _rhs(rhs) {}
...
void EuclideanExpressionNodeElementwiseMultiplyEuclidean::
    getDesignVariablesImplementation(
        DesignVariable::set_t& designVariables) const {
  _lhs->getDesignVariables(designVariables);
  _rhs->getDesignVariables(designVariables);
...
Eigen::Vector3d
EuclideanExpressionNodeElementwiseMultiplyEuclidean::toEuclideanImplementation()
    const {
...
void EuclideanExpressionNodeElementwiseMultiplyEuclidean::
    evaluateJacobiansImplementation(
        ...
        const Eigen::MatrixXd& applyChainRule) const {
  _lhs->evaluateJacobians(outJacobians,
                          applyChainRule * _rhs->toEuclidean().asDiagonal());
  _rhs->evaluateJacobians(outJacobians,
                          applyChainRule * _lhs->toEuclidean().asDiagonal());
}
