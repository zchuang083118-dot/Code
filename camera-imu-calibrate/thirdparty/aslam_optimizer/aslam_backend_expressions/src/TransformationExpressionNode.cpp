...
void TransformationExpressionNode::evaluateJacobians(
...
void TransformationExpressionNode::evaluateJacobians(
  ...
  SM_ASSERT_EQ_DBG(Exception, applyChainRule.cols(), 6,
                   "The chain rule matrix must have 6 columns");
  evaluateJacobiansImplementation(outJacobians, applyChainRule);
}

void TransformationExpressionNode::getDesignVariables(
...
TransformationExpressionNodeMultiply::TransformationExpressionNodeMultiply(
    ...
    std::shared_ptr<TransformationExpressionNode> rhs)
    : _lhs(lhs), _rhs(rhs) {
...
Eigen::Matrix4d
TransformationExpressionNodeMultiply::toTransformationMatrixImplementation() {
  _T_lhs = _lhs->toTransformationMatrix();
  _T_rhs = _rhs->toTransformationMatrix();
...
TransformationExpressionNodeInverse::TransformationExpressionNodeInverse(
    std::shared_ptr<TransformationExpressionNode> dvTransformation)
...
Eigen::Matrix4d
TransformationExpressionNodeInverse::toTransformationMatrixImplementation() {
  _T = _dvTransformation->toTransformationMatrix().inverse();
  return _T;
}

void TransformationExpressionNodeInverse::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
  _dvTransformation->evaluateJacobians(outJacobians,
                                       -sm::kinematics::boxTimes(_T));
}

void TransformationExpressionNodeInverse::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians,
    const Eigen::MatrixXd& applyChainRule) const {
  _dvTransformation->evaluateJacobians(
      outJacobians, (applyChainRule * -sm::kinematics::boxTimes(_T)));
}
...
Eigen::Matrix4d
TransformationExpressionNodeConstant::toTransformationMatrixImplementation() {
  return _T;
}
