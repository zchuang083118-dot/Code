...
MatrixExpression::MatrixExpression(
    MatrixExpressionNode* rotationDesignVariable) {
...
MatrixExpression::MatrixExpression(std::shared_ptr<MatrixExpressionNode> node)
    : _root(node) {
  SM_ASSERT_TRUE(Exception, _root.get() != NULL,
                 "It is illegal to initialized a matrix transformation "
                 "expression with a null node");
}
...
void MatrixExpression::evaluateJacobians(
    JacobianContainer& outJacobians) const {
  _root->evaluateJacobians(outJacobians);
}

EuclideanExpression MatrixExpression::operator*(
...
void MatrixExpression::getDesignVariables(
