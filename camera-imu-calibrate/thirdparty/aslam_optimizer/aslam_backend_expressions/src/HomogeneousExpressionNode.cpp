...
void HomogeneousExpressionNode::evaluateJacobians(
...
void HomogeneousExpressionNode::getDesignVariables(
...
void HomogeneousExpressionNodeMultiply::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
...
void HomogeneousExpressionNodeMultiply::evaluateJacobiansImplementation(
  ...
  _lhs->evaluateJacobians(
      outJacobians, applyChainRule * sm::kinematics::boxMinus(_T_lhs * _p_rhs));
  _rhs->evaluateJacobians(outJacobians, applyChainRule * _T_lhs);
}

void HomogeneousExpressionNodeMultiply::getDesignVariablesImplementation(
    DesignVariable::set_t& designVariables) const {
...
Eigen::Vector4d HomogeneousExpressionNodeConstant::toHomogeneousImplementation()
    const {
  return _p;
}
...
Eigen::Vector4d
HomogeneousExpressionNodeEuclidean::toHomogeneousImplementation() const {
  return sm::kinematics::toHomogeneous(_p->toEuclidean());
}

void HomogeneousExpressionNodeEuclidean::evaluateJacobiansImplementation(
    JacobianContainer& outJacobians) const {
...
void HomogeneousExpressionNodeEuclidean::evaluateJacobiansImplementation(
  ...
  _p->evaluateJacobians(outJacobians,
                        applyChainRule * Eigen::MatrixXd::Identity(4, 3));
}
