...
TEST(RotationExpressionNodeTestSuites, testQuatMultiply2) {
  try {
    ...
    sm::eigen::assertEqual(qr0.toRotationMatrix() * qr1.toRotationMatrix(),
                           dvMultiply.toRotationMatrix(), SM_SOURCE_FILE_POS,
...
TEST(RotationExpressionNodeTestSuites, testQuatMultiply3a) {
  try {
    ...
    sm::eigen::assertNear(qr2.toRotationMatrix() * qr0.toRotationMatrix() *
                              qr1.toRotationMatrix(),
                          dvMultiply.toRotationMatrix(), 1e-14,
...
/// Test that 3x multiplication (q0,q1) q2 and q0 (q1 q2) produce the same
/// result
TEST(RotationExpressionNodeTestSuites, stQuatMulty3b) {
  try {
    ...
    sm::eigen::assertNear(dvMultiply02Left.toRotationMatrix(),
                          dvMultiply02Right.toRotationMatrix(), 1e-14,
                          SM_SOURCE_FILE_POS, "Test multiplications");
    ...
    sm::eigen::assertNear(JcLeft.asSparseMatrix(), JcRight.asSparseMatrix(),
                          1e-14, SM_SOURCE_FILE_POS, "Test multiplications");
...
TEST(RotationExpressionNodeTestSuites, stQuatMulty3c) {
  try {
    ...
    sm::eigen::assertNear(qr0.toRotationMatrix() * qr1.toRotationMatrix() *
                              qr2.toRotationMatrix(),
                          dvMultiplyRight.toRotationMatrix(), 1e-14,
                          SM_SOURCE_FILE_POS, "Test multiplications");
...
TEST(RotationExpressionNodeTestSuites, stQuatMultInv1) {
  try {
    ...
    sm::eigen::assertNear(
        qr2.toRotationMatrix() *
            (qr0.toRotationMatrix() * qr1.toRotationMatrix()).transpose(),
        dvMultiplyRight210.toRotationMatrix(), 1e-14, SM_SOURCE_FILE_POS,
        "Test multiplications");
...
TEST(RotationExpressionNodeTestSuites, stQuatMultInv2) {
  try {
    ...
    sm::eigen::assertNear(qr2.toRotationMatrix().transpose() *
                              (qr0.toRotationMatrix() * qr1.toRotationMatrix()),
                          dvMultiplyRight210.toRotationMatrix(), 1e-14,
                          SM_SOURCE_FILE_POS, "Test multiplications");
...
TEST(RotationExpressionNodeTestSuites, testMinimalDifferenceAndJacobian) {
  try {
    ...
    // std::cout << "initial value is " << std::endl << quatInitial <<
    // std::endl;
    ...
    Eigen::Vector3d realMinDist = sm::kinematics::qlog(
        sm::kinematics::qplus(quatFinal, sm::kinematics::quatInv(quatInitial)));
    ...
    sm::eigen::assertNear(realMinDist, estMinDist, 1e-2, SM_SOURCE_FILE_POS,
                          "Test min difference with jacobian");
  } catch (const std::exception& e) {
    FAIL() << e.what();
  }
}
...
// Test that the binary multiplication produces the correct result.
TEST(RotationExpressionNodeTestSuites,
     testVector2RotationQuaternionExpressionAdapter) {
  try {
    using namespace sm::kinematics;
    RotationQuaternion quat0(quatRandom());
    quat0.setActive(true);
    ...
    RotationExpression qr1 = Vector2RotationQuaternionExpressionAdapter::adapt(
        VectorExpression<4>(&quat1));
    sm::eigen::assertEqual(qr1.toRotationMatrix(), quat1Var.toRotationMatrix(),
                           SM_SOURCE_FILE_POS,
