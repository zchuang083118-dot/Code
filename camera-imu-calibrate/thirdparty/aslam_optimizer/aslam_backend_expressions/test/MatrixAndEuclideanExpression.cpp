...
TEST(EuclideanExpressionNodeTestSuites, testRotatedPoint) {
  try {
    ...
    sm::eigen::assertNear(
        Cp.toEuclidean(), C.toRotationMatrix() * p.toEuclidean(), 1e-14,
...
TEST(EuclideanExpressionNodeTestSuites, testRotatedInverse) {
  try {
    ...
    sm::eigen::assertNear(
        Cp.toEuclidean(), C.toRotationMatrix().transpose() * p.toEuclidean(),
...
TEST(EuclideanExpressionNodeTestSuites, testRotationExpression1) {
  try {
    ...
    sm::eigen::assertNear(
        C01p.toEuclidean(),
        C0.toRotationMatrix() * C1.toRotationMatrix() * p.toEuclidean(), 1e-14,
        SM_SOURCE_FILE_POS, "Testing the result is unchanged");
  } catch (std::exception const& e) {
    FAIL() << e.what();
  }
...
TEST(EuclideanExpressionNodeTestSuites, testNegation) {
  try {
    ...
    sm::eigen::assertNear(-point1.toEuclidean(), pNegated.toEuclidean(), 1e-14,
                          SM_SOURCE_FILE_POS,
...
TEST(EuclideanExpressionNodeTestSuites, testEuclideanDirection) {
  try {
    ...
    ASSERT_DOUBLE_MX_EQ(
        p, point1.toEuclidean(), 1e-2,
        "Checking if the euclidean point is recovered correctly")
...
TEST(EuclideanExpressionNodeTestSuites, testMatrixTransformedPoint) {
  try {
    ...
    sm::eigen::assertNear(Ap.toEuclidean(), A.toMatrix3x3() * p.toEuclidean(),
                          1e-14, SM_SOURCE_FILE_POS,
                          "Testing the result is unchanged");
  } catch (std::exception const& e) {
    FAIL() << e.what();
  }
...
TEST(EuclideanExpressionNodeTestSuites, testMatrixUpdate) {
  try {
    ...
    sm::eigen::assertNear(a.toMatrix3x3(), dataMatrix + updateMatrix, 1e-14,
                          SM_SOURCE_FILE_POS,
                          "Testing the result is unchanged");
  } catch (std::exception const& e) {
    FAIL() << e.what();
...
TEST(EuclideanExpressionNodeTestSuites, testMatrixdoubleTransformedPoint) {
  try {
    ...
    sm::eigen::assertNear(
        Ap.toEuclidean(), B.toMatrix3x3() * (A.toMatrix3x3() * p.toEuclidean()),
        1e-14, SM_SOURCE_FILE_POS, "Testing the result is unchanged");
  } catch (std::exception const& e) {
    FAIL() << e.what();
  }
...
// Test that the jacobian matches the finite difference jacobian
TEST(EuclideanExpressionNodeTestSuites,
     testLowerTriangleMatrixTransformedPoint) {
  try {
...
TEST(EuclideanExpressionNodeTestSuites, testLowerMixedMatrixTransformedPoint) {
  try {
    ...
    a.setBlockIndex(0);
    MatrixExpression A(&a);
    sm::eigen::assertNear(M, A.toMatrix3x3(), 1e-14, SM_SOURCE_FILE_POS,
                          "Testing the initial matrix is good");
    ...
    sm::eigen::assertNear(S, B.toMatrix3x3(), 1e-14, SM_SOURCE_FILE_POS,
                          "Testing the result is unchanged");
    ...
    sm::eigen::assertNear(
        Ap.toEuclidean(), B.toMatrix3x3() * A.toMatrix3x3() * p.toEuclidean(),
        1e-13, SM_SOURCE_FILE_POS, "Testing the result is unchanged");
  } catch (std::exception const& e) {
    FAIL() << e.what();
  }
}
...
TEST(EuclideanExpressionNodeTestSuites, testTransformationTransformedPoint) {
  try {
    ...
    SCOPED_TRACE("");
    sm::eigen::assertNear(
        p.toEuclidean(), A.toTransformationMatrix().topRightCorner<3, 1>(),
        1e-14, SM_SOURCE_FILE_POS, "Testing the result is unchanged");
  } catch (std::exception const& e) {
    FAIL() << e.what();
  }
...
TEST(EuclideanExpressionNodeTestSuites, testRotationParameters) {
  try {
    ...
    SCOPED_TRACE("");
    sm::eigen::assertNear(
        p.toEuclidean(), ypr->rotationMatrixToParameters(qr.toRotationMatrix()),
        1e-14, SM_SOURCE_FILE_POS, "Testing the result is unchanged");
  } catch (std::exception const& e) {
    FAIL() << e.what();
  }
