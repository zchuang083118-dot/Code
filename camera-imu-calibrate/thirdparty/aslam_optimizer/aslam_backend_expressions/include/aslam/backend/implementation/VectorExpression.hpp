...
VectorExpression<D>::VectorExpression(
    std::shared_ptr<VectorExpressionNode<D> > root)
    : _root(root) {}
...
template <int D>
void VectorExpression<D>::evaluateJacobians(
...
template <int D>
void VectorExpression<D>::getDesignVariables(
...
ScalarExpression VectorExpression<D>::toScalarExpression() const {
  ...
  std::shared_ptr<ScalarExpressionNode> newRoot(
      new ScalarExpressionNodeFromVectorExpression(_root));
