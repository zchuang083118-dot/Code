...
class GenericMatrixExpressionNode {
 ...
 protected:
  ...
  inline GenericMatrixExpressionNode(const matrix_t& value)
      : _currentValue(value), _valueDirty(false) {}

 public:
  GenericMatrixExpressionNode(int rows = IRows, int cols = ICols,
...
template <int IRows, int ICols, typename TScalar>
class ConstantGenericMatrixExpressionNode
    : public GenericMatrixExpressionNode<IRows, ICols, TScalar> {
 public:
 ...
 protected:
  ...
  virtual void evaluateJacobiansImplementation(
      JacobianContainer& /* outJacobians */,
      const typename base_t::differential_t& /* chainRuleDifferentail */)
      const {}
  virtual void getDesignVariablesImplementation(
