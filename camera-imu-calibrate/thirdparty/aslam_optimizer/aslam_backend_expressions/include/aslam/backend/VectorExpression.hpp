...
class VectorExpression {
 public:
  ...
  VectorExpression(std::shared_ptr<VectorExpressionNode<D> > root);
  ...
  std::shared_ptr<VectorExpressionNode<D> > root() const { return _root; }

 private:
  std::shared_ptr<VectorExpressionNode<D> > _root;
};
