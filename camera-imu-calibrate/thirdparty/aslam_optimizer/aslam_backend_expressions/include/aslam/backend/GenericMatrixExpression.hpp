...
class GenericMatrixExpression {
 public:
  ...
  typedef TNode node_t;
  typedef GenericMatrixExpression<IRows, ICols, TScalar, default_node_t>
      default_self_t;
  ...
  /// \brief Initialize a constant expression from an matrix. The matrix will be
  /// copied.
  ...
  /// \brief return the expression representing the Moore-Penrose-Inverse of
  /// this expression.
  ...
  GenericMatrixExpression<IRows, IColsOther, TScalar> operator*(
      const GenericMatrixExpression<ICols, IColsOther, TScalar, TOtherNode>
          &other) const;
  ...
  default_self_t operator+(
      const GenericMatrixExpression<IRows, ICols, TScalar, TOtherNode> &other)
      const;
  template <typename TOtherNode>
  default_self_t operator-(
      const GenericMatrixExpression<IRows, ICols, TScalar, TOtherNode> &other)
  ...
  template <int IColsOther, typename TOtherNode,
            typename std::enable_if<
                IRows == 3 && (ICols == 1 || IColsOther == 1), int>::type = 0>
  GenericMatrixExpression<3, (ICols == 1 ? IColsOther : ICols), TScalar> cross(
      const GenericMatrixExpression<3, IColsOther, TScalar, TOtherNode> &other)
      const;
  ...
  void evaluateJacobians(JacobianContainer &outJacobians,
                         const differential_t &diff) const;
 ...
 public:
  ...
  class UnaryOperationResult : public node_t {
   public:
    ...
    inline apply_diff_return_t applyDiff(
        const typename operand_t::tangent_vector_t & /* tangent_vector */)
        const {
   ...
   protected:
    ...
    virtual void evaluateJacobiansImplementation(
        ...
        const typename node_t::differential_t &chainRuleDifferentail) const {
   ...
   private:
    ...
    class Diff
        : public ComposedDifferential<typename operand_t::tangent_vector_t,
                                      apply_diff_return_t, TScalar, Diff> {
      const TDerived &_derivedNode;

     public:
      typedef ComposedDifferential<typename operand_t::tangent_vector_t,
                                   apply_diff_return_t, TScalar, Diff>
          base_t;
      typedef typename base_t::domain_t domain_t;

      inline Diff(const TDerived &derivedNode,
                  const typename TDerived::differential_t &diff)
          : base_t(diff), _derivedNode(derivedNode) {}
      ~Diff() {}

      inline apply_diff_return_t apply(
          const typename operand_t::tangent_vector_t &tangent_vector) const {
  ...
  class BinaryOperationResult : public node_t {
   public:
    ...
    inline apply_diff_return_t applyLhsDiff(
        const typename lhs_t::tangent_vector_t &tangent_vector) const {
      throw std::runtime_error("This method must be shadowed or not used!");
    }
    inline apply_diff_return_t applyRhsDiff(
        const typename rhs_t::tangent_vector_t &tangent_vector) const {
      throw std::runtime_error("This method must be shadowed or not used!");
    }
   ...
   protected:
    ...
    virtual void evaluateJacobiansImplementation(
      ...
      this->getLhsNode().evaluateJacobians(
          outJacobians, Diff<lhs_t, &TDerived::applyLhsDiff>(
                            *static_cast<const TDerived *>(this), diff));
      this->getRhsNode().evaluateJacobians(
          outJacobians, Diff<rhs_t, &TDerived::applyRhsDiff>(
                            *static_cast<const TDerived *>(this), diff));
    }
   ...
   private:
    ...
    template <typename TSide,
              apply_diff_return_t (TDerived::*FApply)(
                  const typename TSide::tangent_vector_t &) const>
    class Diff : public ComposedDifferential<typename TSide::tangent_vector_t,
                                             apply_diff_return_t, TScalar,
                                             Diff<TSide, FApply> > {
      const TDerived &_derivedNode;

     public:
      typedef ComposedDifferential<typename TSide::tangent_vector_t,
                                   apply_diff_return_t, TScalar, Diff>
          base_t;
      typedef typename base_t::domain_t domain_t;
      ...
      inline apply_diff_return_t apply(
          const typename TSide::tangent_vector_t &tangent_vector) const {
