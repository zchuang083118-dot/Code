...
template <typename TExpression>
typename internal::GenericMatrixTraits<TExpression>::gme_expression_type
convertToGME(const TExpression &t) {
  return
      typename internal::GenericMatrixTraits<TExpression>::gme_expression_type(
          const_cast<TExpression &>(t).root());
}
  ...
  class Constant : VectorExpressionNode<D> {
   public:
    ...
    Constant(int rows = D, int cols = 1) {
      if (D != Eigen::Dynamic) {
        SM_ASSERT_EQ_DBG(std::runtime_error, rows, D,
                         "dynamic size has to equal static size");
      }
      SM_ASSERT_EQ_DBG(
          std::runtime_error, cols, 1,
          "there is only one column supported as vector expression.");
    }
...
struct GenericMatrixTraits<VectorExpression<D> > {
  typedef GenericMatrixExpression<D, 1, double, VectorExpressionNode<D> >
      gme_expression_type;
};
