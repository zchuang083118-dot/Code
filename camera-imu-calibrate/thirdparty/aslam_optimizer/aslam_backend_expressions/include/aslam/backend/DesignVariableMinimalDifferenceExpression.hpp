...
namespace backend {
template <int D>
class DesignVariableMinimalDifferenceExpressionNode
    : public VectorExpressionNode<D> {
 public:
  typedef VectorExpressionNode<D> Base;
  typedef typename Base::vector_t vector_t;

  DesignVariableMinimalDifferenceExpressionNode(DesignVariable& dv,
...
class DesignVariableMinimalDifferenceExpression : public VectorExpression<D> {
 public:
  DesignVariableMinimalDifferenceExpression(DesignVariable& dv,
                                            const Eigen::MatrixXd& xHat)
      : VectorExpression<D>(std::shared_ptr<VectorExpressionNode<D> >(
