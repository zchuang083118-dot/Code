...
struct ExpressionDimensionTraits<
    GenericMatrixExpression<IDimension, 1, TScalar, TNode>> {
  enum { Dimension = IDimension };
};
template <typename TScalar>
...
template <typename TExpression,
          int IDimension =
              internal::ExpressionDimensionTraits<TExpression>::Dimension>
class ExpressionErrorTerm : public aslam::backend::ErrorTermFs<IDimension> {
 public:
  ...
  virtual double evaluateErrorImplementation() {
    auto error = internal::ExpressionToEigenVectorTraits<
        TExpression>::toEigenErrorVector(_expression.evaluate());
    this->setError(error);
...
template <typename TExpression,
          int IDimension =
              internal::ExpressionDimensionTraits<TExpression>::Dimension>
inline ::std::shared_ptr<ExpressionErrorTerm<TExpression, IDimension>>
toErrorTerm(TExpression expression) {
  return ::std::shared_ptr<ExpressionErrorTerm<TExpression, IDimension>>(
...
inline ::std::shared_ptr<ErrorTerm> toErrorTerm(
    TExpression expression, const Eigen::MatrixBase<DERIVED_MATRIX> &invR) {
...
template <typename TExpression,
          int IDimension =
              internal::ExpressionDimensionTraits<TExpression>::Dimension,
          typename DERIVED_MATRIX>
inline ::std::shared_ptr<ErrorTerm> toErrorTermSqrt(
    TExpression expression, const Eigen::MatrixBase<DERIVED_MATRIX> &sqrtInvR) {
  auto errorTerm = toErrorTerm(expression);
