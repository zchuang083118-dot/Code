...
inline double rand1() {
  ...
  static boost::variate_generator<base_generator_type &, boost::uniform_real<> >
...
template <typename M>
sparse_block_matrix::SparseBlockMatrix<M> buildRandomMatrix(
    Eigen::MatrixXi rows, Eigen::MatrixXi cols,
  ...
  for (int cb = 0; cb < Mx.bCols(); cb++) {
    for (int rb = 0; rb < Mx.bRows(); rb++) {
      if (rand1() < probabilityOfABlockBeingFilled) {
        ...
        EXPECT_EQ((matrix_t *)NULL, B)
            << "Block at " << rb << "," << cb << " should be null";
...
TEST(sparse_block_matrixTestSuite, testOperator) {
  ...
  for (int r = 0; r < M.rows(); r++) {
    for (int c = 0; c < M.cols(); c++) {
      ASSERT_EQ(0.0, M(r, c))
          << "The uninitailzed matrix did not return zero at " << r << "," << c;
    }
  }
  ...
  for (int r = 0; r < M.rows(); r++) {
    for (int c = 0; c < M.cols(); c++) {
      if (r >= 2 && r < 4 && c < 2) {
        ...
        ASSERT_EQ(0.0, M(r, c))
            << "The uninitailzed matrix did not return zero at " << r << ","
            << c;
      }
    }
  }
...
TEST(sparse_block_matrixTestSuite, testClone) {
  ...
  sparse_block_matrix::SparseBlockMatrix<MatrixXd> ME =
      buildRandomMatrix<MatrixXd>(rows, cols, -1.0);
  ...
  sparse_block_matrix::SparseBlockMatrix<MatrixXd> M1 =
      buildRandomMatrix<MatrixXd>(rows, cols, 0.5);
...
// false on failure template <class MatrixTransposedType> bool
// transpose(SparseBlockMatrix<MatrixTransposedType>*& dest) const;
TEST(sparse_block_matrixTestSuite, testTranspose) {
  ...
  sparse_block_matrix::SparseBlockMatrix<MatrixXd> M2t(
      M1t->rowBlockIndices(), M1t->colBlockIndices(), true);
  ...
  sparse_block_matrix::SparseBlockMatrix<MatrixXd> M3t =
      buildRandomMatrix<MatrixXd>(cols, rows, 0.5);
  ...
  {
    SCOPED_TRACE("");
    sm::eigen::assertEqual(M3.transpose().toDense(),
                           M3.toDense().transpose().eval(), SM_SOURCE_FILE_POS);
  }
}
...
TEST(sparse_block_matrixTestSuite, testScaleMatrix) {
  ...
  try {
    sm::eigen::assertNear((M1 * scale).toDense(), (M1.toDense() * scale).eval(),
...
TEST(sparse_block_matrixTestSuite, testMatrixMatrixMultiplication) {
  ...
  // Create an not empty matrix.
  sparse_block_matrix::SparseBlockMatrix<MatrixXd> M2 =
      buildRandomMatrix<MatrixXd>(cols, rows, 0.5);

  try {
    sm::eigen::assertNear((M1 * M2).toDense(),
                          (M1.toDense() * M2.toDense()).eval(), 1e-6,
                          SM_SOURCE_FILE_POS);
  } catch (const std::exception &e) {
    FAIL() << e.what();
  }
...
// SparseBlockMatrix*  slice(int rmin, int rmax, int cmin, int cmax, bool
// alloc=true) const;
