...
void randomSparseBlockMatrix(
    sparse_block_matrix::SparseBlockMatrix<typename SOLVER_T::matrix_t>* A,
...
void testSolver(const std::string& solver_name) {
  ...
  //  always solve twice to make sure the value only copying is working for
  ...
  sm::eigen::assertNear(dx, xx, 1e-10, SM_SOURCE_FILE_POS,
                        "A: dense solution, B: solution from " + solver_name);
  sm::eigen::assertNear(dx2, xx2, 1e-10, SM_SOURCE_FILE_POS,
                        "A: dense solution, B: solution from " + solver_name);
}
...
TEST(g2oTestSuite, testCholmod) {
  ...
  testSolver<sparse_block_matrix::LinearSolverCholmod<Eigen::MatrixXd> >(
      "cholmod");
