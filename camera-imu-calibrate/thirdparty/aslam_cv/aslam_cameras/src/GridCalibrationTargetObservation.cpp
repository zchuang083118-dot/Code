...
unsigned int GridCalibrationTargetObservation::getCornerReprojection(
  ...
  SM_ASSERT_TRUE(Exception, _T_t_c_isSet,
                 "Extrinsics not set. Reprojection can only be calculated if "
                 "the extrinsics have been calculated. Use findTarget()...");
...
/// the output of getCornersImageFrame and getCornersTargetFrame)
///        returns the number of observed corners
...
void GridCalibrationTargetObservation::updateImagePoint(
  ...
  // SM_DEBUG_STREAM("Updating point " << i << ". Point matrix: (" <<
