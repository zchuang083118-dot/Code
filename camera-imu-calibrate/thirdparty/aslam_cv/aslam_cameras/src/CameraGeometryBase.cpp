...
std::shared_ptr<CameraGeometryBase> CameraGeometryBase::create(
    const sm::PropertyTree& config) {
  std::string type = config.getString("type");

  std::shared_ptr<CameraGeometryBase> rval;
  if (type == "Pinhole") {
    rval.reset(new CameraGeometry<PinholeProjection<NoDistortion>,
                                  GlobalShutter, NoMask>(config));
  } else if (type == "DistortedPinhole") {
    rval.reset(new CameraGeometry<PinholeProjection<RadialTangentialDistortion>,
                                  GlobalShutter, NoMask>(config));
  } else if (type == "EquidistantDistortedPinhole") {
    rval.reset(new CameraGeometry<PinholeProjection<EquidistantDistortion>,
                                  GlobalShutter, NoMask>(config));
  } else if (type == "FovDistortedPinhole") {
    rval.reset(new CameraGeometry<PinholeProjection<FovDistortion>,
                                  GlobalShutter, NoMask>(config));
  } else if (type == "Omni") {
    rval.reset(
    ...
    rval.reset(new CameraGeometry<OmniProjection<RadialTangentialDistortion>,
                                  GlobalShutter, NoMask>(config));
  } else if (type == "EquidistantDistortedOmni") {
    rval.reset(new CameraGeometry<OmniProjection<EquidistantDistortion>,
                                  GlobalShutter, NoMask>(config));
  } else if (type == "ExtendedUnified") {
    rval.reset(new CameraGeometry<ExtendedUnifiedProjection<NoDistortion>,
                                  GlobalShutter, NoMask>(config));
  } else if (type == "DoubleSphere") {
    rval.reset(new CameraGeometry<DoubleSphereProjection<NoDistortion>,
                                  GlobalShutter, NoMask>(config));
    ...
    rval.reset(new CameraGeometry<OmniProjection<NoDistortion>, RollingShutter,
                                  NoMask>(config));
    ...
    rval.reset(new CameraGeometry<OmniProjection<NoDistortion>, GlobalShutter,
                                  ImageMask>(config));
    ...
    SM_THROW(std::runtime_error,
             "Unknown camera type \""
                 ...
                 << "\". Try reading the file "
                    "\"aslam_cv/aslam_cameras/src/CameraGeometryBase.cpp\" for "
                    "a list of valid camera types");
  }
  return rval;
}
