...

{
  ...
  if (rhs._backProjection) {
    _backProjection =
        new sm::kinematics::UncertainVector3(*rhs._backProjection);
  }
}
...
template <int D>
const typename Keypoint<D>::inverse_covariance_t&
Keypoint<D>::inverseMeasurementCovariance() const {
...
/// \brief get the keypoint descriptor
template <int D>
std::shared_ptr<DescriptorBase> Keypoint<D>::descriptorPtr() const {
  return _descriptor;
}
...
template <int D>
void Keypoint<D>::setDescriptorPtr(std::shared_ptr<DescriptorBase> descriptor) {
  _descriptor = descriptor;
}

/// \brief get the landmark
template <int D>
const sm::kinematics::UncertainHomogeneousPoint* Keypoint<D>::landmarkPtr()
    const {
  return _landmark;
}
...
void Keypoint<D>::setRandom() {
  ...
  U.setRandom();
  _inverseMeasurementCovariance =
      U.transpose() * U + inverse_covariance_t::Identity();
  ...
  /// \brief the back projection, a ray in space originating from the view
  /// origin.
  if (sm::random::rand() > 0.5) {
