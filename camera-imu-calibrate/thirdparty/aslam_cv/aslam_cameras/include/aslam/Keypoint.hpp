...
class Keypoint : public KeypointBase {
 public:
  ...
  enum {
    KeypointDimension =
        KEYPOINT_DIMENSION /*!< The dimension of the keypoint associated with
                              this geometry policy */
  };
  ...
  /// \brief get the keypoint descriptor
  virtual std::shared_ptr<DescriptorBase> descriptorPtr() const;
  ...
  /// \brief set the keypoint descriptor
  ///        the keypoint takes ownership of the pointer
  virtual void setDescriptorPtr(std::shared_ptr<DescriptorBase> descriptor);
  ...
  /// \brief Support for unit testing. Is this keypoint binary equal to the
  /// other?
 ...
 private:
  ...
  /// \brief unique ground truth ID for a keypoint trace (i.e. to easily verifiy
  /// 2D2D matching with ground truth simulation)
  size_t _traceId;
};
