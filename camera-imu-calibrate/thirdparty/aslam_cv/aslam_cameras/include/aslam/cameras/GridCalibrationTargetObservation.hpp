 ...
 * \brief an object representing the observation of a gridded calibration
 * pattern in an image
 *
 */
class GridCalibrationTargetObservation {
 public:
  ...
  /// \brief get all corners in image coordinates (order matches
  /// getCornersImageFrame)
  unsigned int getCornersTargetFrame(
      std::vector<cv::Point3f> &outCornerList) const;

  /// \brief get all corners in target frame coordinates (order matches
  /// getObservedTargetFrame)
  unsigned int getCornersImageFrame(
      std::vector<cv::Point2f> &outCornerList) const;

  /// \brief get the point index of all (observed) corners (order corresponds to
  /// the output
  ...
  unsigned int getCornerReprojection(
      const std::shared_ptr<CameraGeometryBase> cameraGeometry,
