...
bool CameraGeometry<P, S, M>::homogeneousToKeypoint(
    ...
    const Eigen::MatrixBase<DERIVED_K> &outKeypoint) const {
  return static_cast<const CameraGeometry<projection_t, shutter_t, mask_t> *>(
             this)
...
// CameraGeometry<P,S,M>::Duration(0).
template <typename P, typename S, typename M>
template <typename DERIVED_K>
...
template <typename P, typename S, typename M>
std::shared_ptr<typename CameraGeometry<P, S, M>::frame_t>
CameraGeometry<P, S, M>::createUninitializedFrame() const {
...
CameraGeometry<P, S, M>::createUninitializedFrameBase() const {
