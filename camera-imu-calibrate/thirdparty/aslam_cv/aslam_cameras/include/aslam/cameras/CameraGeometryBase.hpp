...
// The base class for camera geometry...this will help exporting things to
// python or supporting runtime polymorphism but it will be a bit slower than
// pure templated code.
class CameraGeometryBase {
 public:
  ...
  // The functions below take variable sized matrices and hence they are
  // prefixed by "vs" to avoid confusion with the fixed-size matrix versions
  // available in derived classes. The default implementation of these "vs"
  // functions will be inefficient as it will call the fixed-size derived
  // functions and copy the data into the variable size matrices. Still, this
  // base class is useful as it allows us to maintain a heterogeneous list of
  // camera geometries and simplify the python interface.
  ...
  virtual bool initializeIntrinsics(
      const std::vector<GridCalibrationTargetObservation> &observations) = 0;
  ...
  /// \brief create an uninitialized frame base object. This object has the
  /// right
  ///        type for the underlying camera system but does not have the camera
  ///        system set.
  virtual std::shared_ptr<FrameBase> createUninitializedFrameBase() const = 0;

  // The amount of time elapsed between the start of the image and the
  // keypoint. For a global shutter camera, this can return Duration(0).
  virtual Duration temporalOffset(const Eigen::VectorXd &keypoint) const = 0;

  static std::shared_ptr<CameraGeometryBase> create(
  ...
  virtual void euclideanToKeypointIntrinsicsJacobian(
      ...
      bool estimateDistortion, bool estimateShutter) const = 0;
  ...
  virtual void temporalOffsetIntrinsicsJacobian(const Eigen::VectorXd &keypoint,
                                                Eigen::MatrixXd &outJi,
