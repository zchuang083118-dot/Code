...
class FrameBase {
 public:
  ...
  virtual std::shared_ptr<cameras::CameraGeometryBase> geometryBase() = 0;
  ...
  virtual void setGeometryBase(
      const std::shared_ptr<cameras::CameraGeometryBase>& geometry) = 0;
  ...
  // Should we also implement drawing keypoints on the image using the camera
  // model?
  ...
  virtual std::shared_ptr<const aslam::Image> getOctaves() const = 0;

  // get the image octaves. The underlying aslam::Image non const
  virtual std::shared_ptr<aslam::Image> getOctavesMutable() = 0;
