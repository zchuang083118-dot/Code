...
bool GridCalibrationTargetAprilgrid::computeObservation(
  ...
  for (iter = detections.begin(); iter != detections.end();) {
    ...
    // delete flagged tags
    if (remove) {
      SM_DEBUG_STREAM("Tag with ID "
                      << iter->id
                      << " is only partially in image (corners outside) and "
                         "will be removed from the TargetObservation.\n");
  ...
  for (unsigned int i = 0; i < detections.size(); i++) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      if (subpix_displacement_squarred <= _options.maxSubpixDisplacement2) {
        ...
        SM_DEBUG_STREAM("Subpix refinement failed for point: "
                        << pIdx[j] << " with displacement: "
...
BOOST_CLASS_EXPORT_IMPLEMENT(
    aslam::cameras::GridCalibrationTargetAprilgrid::AprilgridOptions);
