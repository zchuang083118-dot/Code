...
class NCameraSystemDesignVariableContainer {
 public:
  typedef std::shared_ptr<NCameraSystemDesignVariableContainer> Ptr;
  ...
  getCameraGeometryDesignVariableContainer(size_t cameraIndex);
 ...
 private:
  ...
  std::vector<std::shared_ptr<backend::MappedRotationQuaternion> > _q_v_c;
  /// \brief Extrinsics translation design variables
  std::vector<std::shared_ptr<backend::MappedEuclideanPoint> > _p_v_c;
  ...
  std::vector<std::shared_ptr<CameraGeometryDesignVariableContainer> >
      _cameraDvs;
