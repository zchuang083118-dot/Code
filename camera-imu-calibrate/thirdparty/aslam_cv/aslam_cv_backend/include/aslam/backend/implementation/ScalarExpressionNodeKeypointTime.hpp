...
ScalarExpressionNodeKeypointTime<CAMERA_T>::ScalarExpressionNodeKeypointTime(
    const aslam::Time& stamp, const Eigen::VectorXd& y,
    std::shared_ptr<backend::CameraDesignVariable<CAMERA_T> > dv)
...
ScalarExpressionNodeKeypointTime<
    CAMERA_T>::~ScalarExpressionNodeKeypointTime() {}

template <typename CAMERA_T>
double ScalarExpressionNodeKeypointTime<CAMERA_T>::toScalarImplementation()
    const {
  return (_stamp + _dv->camera()->temporalOffset(_y)).toSec();
}

template <typename CAMERA_T>
void ScalarExpressionNodeKeypointTime<
    CAMERA_T>::evaluateJacobiansImplementation(backend::JacobianContainer&
                                                   outJacobians) const {
  Eigen::MatrixXd Ji;
...
template <typename CAMERA_T>
void ScalarExpressionNodeKeypointTime<CAMERA_T>::
...
template <typename CAMERA_T>
void ScalarExpressionNodeKeypointTime<
    CAMERA_T>::getDesignVariablesImplementation(backend::DesignVariable::set_t&
                                                    designVariables) const {
