...
CameraDesignVariable<CAMERA_T>::CameraDesignVariable(
    const std::shared_ptr<CAMERA_T>& camera)
    : _camera(camera) {
  // create the design variables:
  _projectionDv.reset(
      new DesignVariableAdapter<projection_t>(&_camera->projection(), false));
  _distortionDv.reset(new DesignVariableAdapter<distortion_t>(
      &((_camera->projection()).distortion()), false));
  _shutterDv.reset(
      new DesignVariableAdapter<shutter_t>(&_camera->shutter(), false));
}
...
template <typename CAMERA_T>
Eigen::VectorXd CameraDesignVariable<CAMERA_T>::euclideanToKeypoint(
    Eigen::Vector3d p) {
  keypoint_t kp;
...
template <typename CAMERA_T>
Eigen::VectorXd CameraDesignVariable<CAMERA_T>::homogeneousToKeypoint(
    Eigen::Vector4d ph) {
  keypoint_t kp;
...
template <typename CAMERA_T>
void CameraDesignVariable<CAMERA_T>::evaluateJacobians(
    JacobianContainer& outJacobians, Eigen::Vector4d ph) const {
  Eigen::MatrixXd Jp;
  Eigen::MatrixXd Jd;
  Eigen::MatrixXd Js;
...
template <typename CAMERA_T>
void CameraDesignVariable<CAMERA_T>::evaluateJacobians(
    JacobianContainer& outJacobians, const Eigen::MatrixXd& applyChainRule,
...
template <typename CAMERA_T>
void CameraDesignVariable<CAMERA_T>::getDesignVariables(
  ...
  if (_shutterDv->isActive()) {
    std::shared_ptr<backend::ScalarExpressionNode> root(
        new ScalarExpressionNodeKeypointTime<CAMERA_T>(
            imageStamp, y, this->shared_from_this()));
