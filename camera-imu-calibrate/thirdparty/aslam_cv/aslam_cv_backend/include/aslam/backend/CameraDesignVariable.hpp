...
class CameraDesignVariable
 ...
 public:
  ...
  CameraDesignVariable(const std::shared_ptr<camera_t>& camera);
  ...
  std::shared_ptr<DesignVariableAdapter<projection_t> >
  projectionDesignVariable() {
    return _projectionDv;
  }
  std::shared_ptr<DesignVariableAdapter<distortion_t> >
  distortionDesignVariable() {
    return _distortionDv;
  }
  std::shared_ptr<DesignVariableAdapter<shutter_t> > shutterDesignVariable() {
    return _shutterDv;
  }

  std::shared_ptr<camera_t> camera() const { return _camera; }
 ...
 private:
  ...
  std::shared_ptr<DesignVariableAdapter<projection_t> > _projectionDv;
  std::shared_ptr<DesignVariableAdapter<distortion_t> > _distortionDv;
  std::shared_ptr<DesignVariableAdapter<shutter_t> > _shutterDv;
};
