...
class CameraGeometryDesignVariableContainer {
 public:
  ...
  /// \brief Get the temporal offset from the start of the image as an
  /// expression.
  ///        If the shutter parameters are being estimated, this will be hooked
  ///        up to the camera design variable
  ...
  ///        time used to produce p_c should be derived from keypointTime()
  ///        above
  std::shared_ptr<ReprojectionError> createReprojectionError(
  ...
  /// \brief Evaluate the jacobians of the homogeneousToKeypoint() with respect
  ...
  std::shared_ptr<cameras::CameraGeometryBase> camera() { return _camera; }
  ...
  std::shared_ptr<aslam::backend::DesignVariable> getDesignVariable() const {
    return _cameraDv;
  }
 ...
 private:
  ...
  std::shared_ptr<cameras::CameraGeometryBase> _camera;
  std::shared_ptr<
      backend::DesignVariableAdapter<CameraGeometryDesignVariableContainer> >
      _cameraDv;
};
