...
NCameraSystemDesignVariableContainer::NCameraSystemDesignVariableContainer(
  ...
  for (size_t i = 0; i < cameraSystem->numCameras(); ++i) {
    std::shared_ptr<sm::kinematics::Transformation> T_v_c =
        cameraSystem->ptrT_v_c(i);
    _T_c_v[i] =
        aslam::backend::transformationToExpression(*T_v_c, _q_v_c[i], _p_v_c[i])
            .inverse();
  ...
  // CameraGeometryDesignVariableContainer( const
  // std::shared_ptr<cameras::CameraGeometryBase> & camera,
  //                                        bool estimateProjection,
  //                                        bool estimateDistortion,
  //                                        bool estimateShutter );
  for (size_t i = 0; i < cameraSystem->numCameras(); ++i) {
    _cameraDvs[i].reset(new CameraGeometryDesignVariableContainer(
        cameraSystem->geometry(i), estimateProjection, estimateDistortion,
        estimateShutter));
  }
...
void NCameraSystemDesignVariableContainer::getDesignVariables(
  ...
  BOOST_FOREACH (const std::shared_ptr<backend::MappedRotationQuaternion> &q,
                 _q_v_c) {
    dvs.insert(q.get());
  }

  BOOST_FOREACH (const std::shared_ptr<backend::MappedEuclideanPoint> &p,
                 _p_v_c) {
    dvs.insert(p.get());
  }
}
...
void NCameraSystemDesignVariableContainer::setIntrinsicsActive(bool active) {
  BOOST_FOREACH (std::shared_ptr<CameraGeometryDesignVariableContainer> &cam,
                 _cameraDvs) {
...
void NCameraSystemDesignVariableContainer::setExtrinsicsRotationActive(
    bool active) {
  BOOST_FOREACH (std::shared_ptr<backend::MappedRotationQuaternion> &q,
                 _q_v_c) {
    q->setActive(active);
  }
}
...
void NCameraSystemDesignVariableContainer::setExtrinsicsTranslationActive(
    bool active) {
  BOOST_FOREACH (std::shared_ptr<backend::MappedEuclideanPoint> &p, _p_v_c) {
    p->setActive(active);
  }
}
...
backend::TransformationExpression
NCameraSystemDesignVariableContainer::getT_c_v(size_t cameraIndex) {
...
std::shared_ptr<CameraGeometryDesignVariableContainer>
...
NCameraSystemDesignVariableContainer::createReprojectionError(
  ...
  std::shared_ptr<FrameBase> frame = mf.getFrame(kid.cameraIndex);
...
NCameraSystem::Ptr NCameraSystemDesignVariableContainer::getCameraSystem()
    const {
