...
std::shared_ptr<UndistorterBase> UndistorterBase::createUndistorter(
  ...
  std::shared_ptr<UndistorterBase> rval;
  if (type == "NullPinholeCameraGeometry") {
    rval.reset(
        new NullUndistorter<CameraGeometry<PinholeProjection<NoDistortion>,
                                           GlobalShutter, NoMask> >(
    ...
    rval.reset(new NullUndistorter<
               CameraGeometry<PinholeProjection<RadialTangentialDistortion>,
                              GlobalShutter, NoMask> >(undistorterConfig,
                                                       cameraConfig));
  } else if (type == "NullEquidistantDistortedPinholeCameraGeometry") {
    rval.reset(
        new NullUndistorter<CameraGeometry<
            PinholeProjection<EquidistantDistortion>, GlobalShutter, NoMask> >(
            undistorterConfig, cameraConfig));
  } else if (type == "NullOmniCameraGeometry") {
    rval.reset(new NullUndistorter<CameraGeometry<OmniProjection<NoDistortion>,
                                                  GlobalShutter, NoMask> >(
        undistorterConfig, cameraConfig));
    ...
    rval.reset(
        new NullUndistorter<CameraGeometry<
            OmniProjection<EquidistantDistortion>, GlobalShutter, NoMask> >(
            undistorterConfig, cameraConfig));
  } else if (type == "NullPinholeRsCameraGeometry") {
    rval.reset(
        new NullUndistorter<CameraGeometry<PinholeProjection<NoDistortion>,
                                           RollingShutter, NoMask> >(
            undistorterConfig, cameraConfig));
  } else if (type == "NullDistortedPinholeRsCameraGeometry") {
    rval.reset(new NullUndistorter<
               CameraGeometry<PinholeProjection<RadialTangentialDistortion>,
                              RollingShutter, NoMask> >(undistorterConfig,
                                                        cameraConfig));
  } else if (type == "NullEquidistantDistortedPinholeRsCameraGeometry") {
    rval.reset(
        new NullUndistorter<CameraGeometry<
            PinholeProjection<EquidistantDistortion>, RollingShutter, NoMask> >(
            undistorterConfig, cameraConfig));
    ...
    rval.reset(
        new NullUndistorter<CameraGeometry<
            OmniProjection<EquidistantDistortion>, RollingShutter, NoMask> >(
            undistorterConfig, cameraConfig));
  } else if (type == "NullMaskedPinholeCameraGeometry") {
    rval.reset(
        new NullUndistorter<CameraGeometry<PinholeProjection<NoDistortion>,
                                           GlobalShutter, ImageMask> >(
            undistorterConfig, cameraConfig));
  } else if (type == "NullMaskedDistortedPinholeCameraGeometry") {
    rval.reset(new NullUndistorter<
               CameraGeometry<PinholeProjection<RadialTangentialDistortion>,
                              GlobalShutter, ImageMask> >(undistorterConfig,
                                                          cameraConfig));
    ...
    rval.reset(
        new NullUndistorter<CameraGeometry<
            OmniProjection<EquidistantDistortion>, GlobalShutter, ImageMask> >(
            undistorterConfig, cameraConfig));
  } else if (type == "NullMaskedPinholeRsCameraGeometry") {
    rval.reset(
        new NullUndistorter<CameraGeometry<PinholeProjection<NoDistortion>,
                                           RollingShutter, ImageMask> >(
            undistorterConfig, cameraConfig));
  } else if (type == "NullMaskedDistortedPinholeRsCameraGeometry") {
    rval.reset(new NullUndistorter<
               CameraGeometry<PinholeProjection<RadialTangentialDistortion>,
                              RollingShutter, ImageMask> >(undistorterConfig,
                                                           cameraConfig));
    ...
    rval.reset(
        new NullUndistorter<CameraGeometry<
            OmniProjection<EquidistantDistortion>, RollingShutter, ImageMask> >(
            undistorterConfig, cameraConfig));
  } else if (type == "MaskedPinholeUndistorter") {
    rval.reset(new PinholeUndistorter<RadialTangentialDistortion, ImageMask>(
        undistorterConfig, cameraConfig));
  } else if (type == "PinholeUndistorter") {
    rval.reset(new PinholeUndistorter<RadialTangentialDistortion, NoMask>(
        undistorterConfig, cameraConfig));
  } else if (type == "MaskedEquidistantPinholeUndistorter") {
    rval.reset(new PinholeUndistorter<EquidistantDistortion, ImageMask>(
        undistorterConfig, cameraConfig));
  } else if (type == "EquidistantPinholeUndistorter") {
    rval.reset(new PinholeUndistorter<EquidistantDistortion, NoMask>(
        undistorterConfig, cameraConfig));
    ...
    SM_THROW(std::runtime_error,
             "Unknown undistorter type \""
                 ...
                 << "\". Try reading the file "
                    "\"aslam_cv/aslam_imgproc/src/UndistorterBase.cpp\" for a "
                    "list of valid undistorters");
  }
