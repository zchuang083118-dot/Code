...
enum InterpolationType {
  ...
  Area = cv::INTER_AREA,  // resampling using pixel area relation. It may be the
  ...
  Lanczos4 =
      cv::INTER_LANCZOS4  // Lanczos interpolation over 8x8 pixel neighborhood
};
...
class UndistorterBase {
 public:
  ...
  static std::shared_ptr<UndistorterBase> createUndistorter(
  ...
  virtual std::shared_ptr<FrameBase> buildFrame(cv::Mat& inImage) = 0;
  ...
  virtual std::shared_ptr<cameras::CameraGeometryBase> idealGeometryBase()
      const = 0;

  /// \brief Get the distorted geometr (before undistortion)
  virtual std::shared_ptr<cameras::CameraGeometryBase> distortedGeometryBase()
      const = 0;
};
