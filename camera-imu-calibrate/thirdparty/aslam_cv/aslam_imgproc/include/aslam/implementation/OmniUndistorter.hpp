...
OmniUndistorter<MASK_T>::OmniUndistorter(
  ...
  int interpolation =
      undistorterConfig.getInt("interpolationType", cv::INTER_NEAREST);
...
template <typename MASK_T>
OmniUndistorter<MASK_T>::OmniUndistorter(
    std::shared_ptr<distorted_geometry_t> distortedGeometry, int interpolation,
...
// intialize a map to convert an undistorted omni to a pinhole image (with
// alpha=1, scale=1.0)
template <typename CAMERA_T>
void initUndistOmniToPinholeMap(std::shared_ptr<CAMERA_T> camera_geometry,
                                const Eigen::Matrix3d& _pinholeCameraMatrix,
                                cv::Size size, cv::OutputArray _map1,
  ...
  for (int i = 0; i < size.height; i++) {
    ...
    double _x = i * invR(0, 1) + invR(0, 2), _y = i * invR(1, 1) + invR(1, 2),
...
template <typename MASK_T>
std::shared_ptr<typename OmniUndistorter<MASK_T>::ideal_geometry_t>
OmniUndistorter<MASK_T>::idealGeometry() const {
  return _idealGeometry;
}

template <typename MASK_T>
std::shared_ptr<typename OmniUndistorter<MASK_T>::ideal_pinhole_geometry_t>
OmniUndistorter<MASK_T>::idealPinholeGeometry() const {
  return _idealPinholeGeometry;
}

template <typename MASK_T>
std::shared_ptr<typename OmniUndistorter<MASK_T>::distorted_geometry_t>
OmniUndistorter<MASK_T>::distortedGeometry() const {
  return _distortedGeometry;
}

template <typename MASK_T>
void OmniUndistorter<MASK_T>::setIdealGeometry(
    std::shared_ptr<distorted_geometry_t> distortedGeometry, double alpha,
    double scale) {
  _distortedGeometry = distortedGeometry;
  Eigen::Matrix3d distortedCameraMatrix =
      distortedGeometry->projection().getCameraMatrix();
  ...
  _idealGeometry = std::shared_ptr<ideal_geometry_t>(
  ...
  cv::initUndistortRectifyMap(
      distortedCameraMatrixCV, distortionParametersCV, cv::Mat(),
      idealCameraMatrixCV, cv::Size(newWidth, newHeight), CV_16SC2, mapX, mapY);
}

template <typename MASK_T>
void OmniUndistorter<MASK_T>::setIdealPinholeGeometry(
    std::shared_ptr<ideal_geometry_t> idealGeometry) {
  ...
  initUndistOmniToPinholeMap(idealGeometry, newCameraMatrix, newsize,
                             mapX_pinhole, mapY_pinhole);

  // create pinhole camera
  typename ideal_pinhole_geometry_t::projection_t idealPinholeProjection(
      newCameraMatrix(0, 0), newCameraMatrix(1, 1), newCameraMatrix(0, 2),
      newCameraMatrix(1, 2), width, height,
      typename ideal_pinhole_geometry_t::projection_t::distortion_t());

  _idealPinholeGeometry =
      std::shared_ptr<ideal_pinhole_geometry_t>(new ideal_pinhole_geometry_t(
          idealPinholeProjection, _distortedGeometry->shutter(),
          _distortedGeometry->mask()));
}

template <typename MASK_T>
std::shared_ptr<FrameBase> OmniUndistorter<MASK_T>::buildFrame(
    cv::Mat& inImage) {
