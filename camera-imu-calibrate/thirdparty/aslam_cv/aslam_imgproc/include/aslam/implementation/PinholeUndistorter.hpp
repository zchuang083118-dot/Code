...
template <typename DISTORTION_T, typename MASK_T>
void PinholeUndistorter<DISTORTION_T, MASK_T>::undistortImage(
    const cv::Mat& inImage, cv::Mat& outImage) const {
  // see:
...
template <typename DISTORTION_T, typename MASK_T>
std::shared_ptr<
    typename PinholeUndistorter<DISTORTION_T, MASK_T>::ideal_geometry_t>
...
template <typename DISTORTION_T, typename MASK_T>
std::shared_ptr<
    typename PinholeUndistorter<DISTORTION_T, MASK_T>::distorted_geometry_t>
...
void PinholeUndistorter<DISTORTION_T, MASK_T>::setIdealGeometry(
  ...
  idealCameraMatrix =
      aslamcv_helper::getOptimalNewCameraMatrix<distorted_geometry_t>(
          _distortedGeometry, cv::Size(width, height), alpha,
          cv::Size(newWidth, newHeight));

  // create idealProjection
  typename ideal_geometry_t::projection_t idealProjection(
      idealCameraMatrix(0, 0), idealCameraMatrix(1, 1), idealCameraMatrix(0, 2),
      idealCameraMatrix(1, 2), newWidth, newHeight,
      typename ideal_geometry_t::projection_t::distortion_t());

  // set new idealGeometry
  _idealGeometry = std::make_shared<ideal_geometry_t>(
      idealProjection, _distortedGeometry->shutter(),
  ...
  aslamcv_helper::initUndistortRectifyMap<distorted_geometry_t>(
      _distortedGeometry, Eigen::Matrix3d::Identity(), idealCameraMatrix,
      cv::Size(newWidth, newHeight), CV_16SC2, mapX, mapY);
}

template <typename DISTORTION_T, typename MASK_T>
std::shared_ptr<FrameBase> PinholeUndistorter<DISTORTION_T, MASK_T>::buildFrame(
    cv::Mat& inImage) {
