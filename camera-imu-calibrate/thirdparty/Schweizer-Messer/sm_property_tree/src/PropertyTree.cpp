...
PropertyTree::PropertyTree(std::shared_ptr<PropertyTreeImplementation> imp,
...
PropertyTree::PropertyTree(const PropertyTree& parent,
...
std::string PropertyTree::buildQualifiedKeyName(const std::string& key) const {
  SM_ASSERT_GT(InvalidKeyException, key.size(), 0,
               "The key must contain characters");
  if (key[0] == '/') {
    // The requested key is in the global namespace
    SM_ASSERT_GT(InvalidKeyException, key.size(), 1,
                 "The key \"" << key
                              << "\" is in the global namespace but does not "
...
double PropertyTree::getDouble(const std::string& key,
...
std::string PropertyTree::getString(const std::string& key,
