 ...
 *   _memberClass1(PropertyTree(config, "member1")),
 * _memberClass2(PropertyTree(config, "member2"))
 * {
 *    // ...
 * }
 * \endcode
 ...
 *    The virtual functions there. This should do the work of actually getting
 * and setting parameter values. All values passed in to these functions will
 * start with "/" and namespaces will be delimited by "/" such as
 * "/namespace1/namespace2/key". If your middlewear/parameter container uses a
 * different delimiter, you may have to find and replace values.
 *
 * - Next create a subclass of PropertyTree that does two things: (1) implements
 * a constructor that calls the PropertyTree constructor instantiating the
 * correct implementation class, and (2) forwards any extra functions from the
 * implementation class that are needed on the outside, for example save/load
 * functionality. It is important that this class stores no data because it will
 * be stripped off when the property tree is passed in to sub-objects.
 *
 *   The classes BoostPropertyTreeImplementation and BoostPropertyTree are good
...
class PropertyTree {
 public:
  ...
  PropertyTree(std::shared_ptr<PropertyTreeImplementation> imp,
 ...
 protected:
  ...
  std::shared_ptr<PropertyTreeImplementation> _imp;
};
