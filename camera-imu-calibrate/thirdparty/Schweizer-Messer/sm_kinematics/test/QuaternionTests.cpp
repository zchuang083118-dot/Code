...
TEST(QuaternionAlgebraTestSuite, testRotation) {
  try {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      sm::eigen::assertNear(
          v_a1, v_a2, 1e-10, SM_SOURCE_FILE_POS,
...
TEST(QuaternionAlgebraTestSuite, testAxisAngle2QuatAndBackOnSpecialValues) {
  ...
  try {
    ...
    for (auto p : testPairs) {
      sm::eigen::assertNear(p.second, axisAngle2quat(p.first * M_PI), eps,
                            SM_SOURCE_FILE_POS, "");
      if (p.second(3) != -1.0) {
        sm::eigen::assertNear(p.first, quat2AxisAngle(p.second) / M_PI, eps,
                              SM_SOURCE_FILE_POS, "");
        sm::eigen::assertNear(p.second,
                              axisAngle2quat(quat2AxisAngle(p.second)), eps,
                              SM_SOURCE_FILE_POS, "");
        sm::eigen::assertNear(p.first, quat2AxisAngle(axisAngle2quat(p.first)),
                              2 * eps, SM_SOURCE_FILE_POS, "");
      }
    }
  } catch (const std::exception& e) {
    FAIL() << e.what();
...
TEST(QuaternionAlgebraTestSuite, testAxisAngle2QuatAndBack) {
  ...
  try {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      sm::eigen::assertNear(q, axisAngle2quat(quat2AxisAngle(q)),
                            q.block<3, 1>(0, 0).norm() * eps * 20,
                            SM_SOURCE_FILE_POS, "");
      ...
      if (i % 2 == 0) {
        if (i % 4 == 0) {
          // make use of the special code for quaternions close to identity more
          // likely!
          v_b *= 1E-20;
        } else {
      ...
      sm::eigen::assertNear(
          v_b, result, tolerance, SM_SOURCE_FILE_POS,
          Stringer()
              << "axisAngle2QuatAndBack is too little accurate : \naA2Q(v_b)="
              << axisAngle2quat(v_b)
  ...
  for (int i = 0; i < 100; i++) {
    ...
    sm::eigen::assertNear(
        ...
        sm::eigen::numericalDiff<Eigen::Vector4d, Eigen::Vector3d>(
            [&v](const Eigen::Vector3d& x) { return axisAngle2quat(v + x); },
            zero3),
        ...
        sm::eigen::numericalDiff<Eigen::Vector3d, Eigen::Vector4d>(
            [&q](const Eigen::Vector4d& x) {
              return quat2AxisAngle(
                  qplus(axisAngle2quat(2 * qeps(qplus(x, quatInv(q)))), q));
            },
            zero4),
        1E-7, SM_SOURCE_FILE_POS, "");
    // TODO figure out how quatLogJacobian was meant to work and write test (the
    // following does not work)
    //  sm::eigen::assertNear(quatLogJacobian(q),
    //  sm::eigen::numericalDiff<Eigen::Vector3d, Eigen::Vector4d >( [&q](const
    //  Eigen::Vector4d & x) { return  quat2AxisAngle(qplus(axisAngle2quat(2 *
    //  qeps(qplus(x, quatInv(q)))), q)); }, zero4), 1E-7, SM_SOURCE_FILE_POS,
    //  "");
  }
}
